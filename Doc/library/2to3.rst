...
presence of the ``kutoka __future__ agiza print_function`` compiler directive, it
...
translation kutoka one directory to another.
...
To translate an entire project kutoka one directory tree to another use:
...
.. 2to3fixer:: future
   ...
   Removes ``kutoka __future__ agiza new_feature`` statements.
...
.. 2to3fixer:: imports2
   ...
   Handles other modules renames in the standard library.  It is separate kutoka
...
.. 2to3fixer:: map
   ...
   to ``list(x)``.  Using ``kutoka future_builtins agiza map`` disables this
   fixer.

.. 2to3fixer:: metaclass

   Converts the old metakundi syntax (``__metaclass__ = Meta`` in the class
   body) to the new (``kundi X(metaclass=Meta)``).
...
.. 2to3fixer:: operator
   ...
   statements are added, e.g. ``agiza collections.abc``.  The following mapping
   are made:
...
.. 2to3fixer:: types
   ...
   Fixes code broken kutoka the removal of some members in the :mod:`types`
   module.
...
.. 2to3fixer:: ws_comma
   ...
   Removes excess whitespace kutoka comma separated items.  This fixer is
   optional.
...
.. 2to3fixer:: zip
   ...
   ``kutoka future_builtins agiza zip`` appears.
