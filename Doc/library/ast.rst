...
classes all inherit kutoka :class:`ast.AST`.  An abstract syntax tree can be
...
.. class:: AST
   ...
   derived kutoka the :file:`Parser/Python.asdl` file, which is reproduced
   :ref:`below <abstract-grammar>`.  They are defined in the :mod:`_ast` C
   module and re-exported in :mod:`ast`.

   There is one kundi defined for each left-hand side symbol in the abstract
   grammar (for example, :class:`ast.stmt` or :class:`ast.expr`).  In addition,
   there is one kundi defined for each constructor on the right-hand side; these
   classes inherit kutoka the classes for the left-hand side trees.  For example,
   :class:`ast.BinOp` inherits kutoka :class:`ast.expr`.  For production rules
   with alternatives (aka "sums"), the left-hand side kundi is abstract: only
   ...
   .. attribute:: _fields
      ...
      Each concrete kundi has an attribute :attr:`_fields` which gives the names
      of all child nodes.

      Each instance of a concrete kundi has one attribute for each child node,
   ...
   The constructor of a kundi :class:`ast.T` parses its arguments as follows:
...
Apart kutoka the node classes, the :mod:`ast` module defines these utility functions
...
.. function:: literal_eval(node_or_string)
   ...
   This can be used for safely evaluating strings containing Python values kutoka
...
.. function:: copy_location(new_node, old_node)
   ...
   and :attr:`end_col_offset`) kutoka *old_node* to *new_node* if possible,
   and return *new_node*.
...
.. class:: NodeTransformer()
   ...
   of the visitor method is ``None``, the node will be removed kutoka its
