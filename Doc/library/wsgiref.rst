...
.. function:: shift_path_info(environ)
   ...
   Shift a single name kutoka ``PATH_INFO`` to ``SCRIPT_NAME`` and return the name.
   ...
   a WSGI application at ``/foo/bar``.  That is, ``SCRIPT_NAME`` will change kutoka
   ``/foo`` to ``/foo/bar``, and ``PATH_INFO`` will change kutoka ``/bar/baz`` to
   ``/baz``.
   ...
   between URIs ending in ``/x`` kutoka ones ending in ``/x/`` when using this
...
.. class:: FileWrapper(filelike, blksize=8192)
   ...
   Example usage::
      ...
      kutoka wsgiref.util agiza FileWrapper
...
request.  (E.g., using the :func:`shift_path_info` function kutoka
:mod:`wsgiref.util`.)
...
.. function:: make_server(host, port, app, server_class=WSGIServer, handler_class=WSGIRequestHandler)
   ...
   Example usage::
      ...
      kutoka wsgiref.simple_server agiza make_server, demo_app
...
.. class:: WSGIServer(server_address, RequestHandlerClass)
   ...
   ``(host,port)`` tuple, and *RequestHandlerClass* should be the subkundi of
   ...
   :class:`WSGIServer` is a subkundi of :class:`http.server.HTTPServer`, so all
...
.. class:: WSGIRequestHandler(request, client_address, server)
   ...
   You do not need to create instances of this kundi directly; they are
   automatically created as needed by :class:`WSGIServer` objects.  You can,
   however, subkundi this kundi and supply it as a *handler_class* to the
   ...
   .. method:: WSGIRequestHandler.get_environ()
      ...
      kutoka the HTTP request.  Each call to this method should return a new dictionary
   ...
   .. method:: WSGIRequestHandler.handle()
      ...
      using a :mod:`wsgiref.handlers` kundi to implement the actual WSGI application
      interface.
...
absence of errors kutoka this module does not necessarily mean that errors do not
...
This module is based on the :mod:`paste.lint` module kutoka Ian Bicking's "Python
Paste" library.
...
.. class:: CGIHandler()
   ...
   This kundi is a subkundi of :class:`BaseCGIHandler` that sets ``wsgi.run_once``
...
.. class:: IISCGIHandler()
   ...
   separate handler kundi is provided.  It is used in the same way as
...
.. class:: BaseCGIHandler(stdin, stdout, stderr, environ, multithread=True, multiprocess=False)
   ...
   This kundi is a subkundi of :class:`SimpleHandler` intended for use with
   ...
   ``Status:`` header to send an HTTP status, you probably want to subkundi this
...
.. class:: SimpleHandler(stdin, stdout, stderr, environ, multithread=True, multiprocess=False)
   ...
   want to subkundi this instead of :class:`BaseCGIHandler`.

   This kundi is a subkundi of :class:`BaseHandler`.  It overrides the
...
.. class:: BaseHandler()
   ...
   This is an abstract base kundi for running WSGI applications.  Each instance
   will handle a single HTTP request, although in principle you could create a
   subkundi that was reusable for multiple requests.
   ...
   .. attribute:: BaseHandler.os_environ
      ...
      at the kundi or instance level.  Note that the dictionary should be considered
   ...
   .. method:: BaseHandler.get_scheme()
      ...
      implementation uses the :func:`guess_scheme` function kutoka :mod:`wsgiref.util`
   ...
   .. method:: BaseHandler.error_output(environ, start_response)
      ...
      Note, however, that it's not recommended kutoka a security perspective to spit out
...
.. function:: read_environ()
   ...
   Transcode CGI variables kutoka ``os.environ`` to :pep:`3333` "bytes in unicode"
...
This is a working "Hello World" WSGI application::
   ...
   # use a kundi for example). The first argument passed to the function
