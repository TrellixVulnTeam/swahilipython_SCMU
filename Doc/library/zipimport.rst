:mod:`zipimport` --- Import modules kutoka Zip archives
=====================================================

.. module:: zipimport
   :synopsis: support for importing Python modules kutoka ZIP archives.
...
This module adds the ability to agiza Python modules (:file:`\*.py`,
:file:`\*.pyc`) and packages kutoka ZIP-format archives. It is usually not
...
and a path within the archive can be specified to only agiza kutoka a
subdirectory.  For example, the path :file:`example.zip/lib/` would only
agiza kutoka the :file:`lib/` subdirectory within the archive.

Any files may be present in the ZIP archive, but only files :file:`.py` and
:file:`.pyc` are available for import.  ZIP agiza of dynamic modules
...
.. seealso::
   ...
   :pep:`273` - Import Modules kutoka Zip Archives
      Written by James C. Ahlstrom, who also provided an implementation. Python 2.3
      follows the specification in :pep:`273`, but uses an implementation written by Just
      van Rossum that uses the agiza hooks described in :pep:`302`.

   :pep:`302` - New Import Hooks
      The PEP to add the agiza hooks that help this module work.
...
.. exception:: ZipImportError
   ...
   Exception raised by zipimporter objects. It's a subkundi of :exc:`ImportError`,
...
:class:`zipimporter` is the kundi for importing ZIP files.
...
Here is an example that imports a module kutoka a ZIP archive - note that the
