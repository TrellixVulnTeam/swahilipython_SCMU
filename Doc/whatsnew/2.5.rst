...
Here's another example, kutoka a program that uses PyGTK.  Here a context-sensitive
...
be used to enclose the names imported kutoka a module using the ``kutoka ... import
...`` statement, making it easier to agiza many different names.
...
the statement ``agiza string``?  In Python 2.4 and earlier, it will first look
...
:mod:`py.std` package provides a tidier way to perform imports kutoka the standard
library, ``agiza py; py.std.string.join()``, but that package isn't available
...
using a ``kutoka __future__ agiza absolute_import`` directive.  This absolute-import
behaviour will become the default in a future version (probably Python
2.7).  Once absolute imports  are the default, ``agiza string`` will always
...
``kutoka pkg agiza string`` in your code.

Relative imports are still possible by adding a leading period  to the module
name when using the ``kutoka ... import`` form::

   # Import names kutoka pkg.string
   kutoka .string agiza name1, name2
   # Import pkg.string
   kutoka . agiza string

This imports the :mod:`string` module relative to the current package, so in
:mod:`pkg.main` this will agiza *name1* and *name2* kutoka :mod:`pkg.string`.
Additional leading periods perform the relative agiza starting kutoka the parent
of the current package.  For example, code in the :mod:`A.B.C` module can do::

   kutoka . agiza D                 # Imports A.B.D
   kutoka .. agiza E                # Imports A.E
   kutoka ..F agiza G               # Imports A.F.G

Leading periods cannot be used with the ``agiza modname``  form of the import
statement, only the ``kutoka ... import`` form.
...
The :mod:`runpy` module implements a more sophisticated agiza mechanism so that
it's now possible to run modules in a package such as :mod:`pychecker.checker`.
The module also supports alternative agiza mechanisms such as the
:mod:`zipimport` module.  This means you can add a .zip archive's path to
``sys.path`` and then use the :option:`-m` switch to execute code kutoka the
archive.
...
kutoka 0 up to 9.  On encountering the :keyword:`yield` statement, the iterator
...
.. seealso::
   ...
   http://www.sidhe.org/~dan/blog/archives/000178.html
      An explanation of coroutines kutoka a Perl point of view, written by Dan Sugalski.
...
to your module::
   ...
   kutoka __future__ agiza with_statement
...
Our database example kutoka the previous section could be written  using this
decorator as::
...
the built-in :exc:`Exception` kundi and all the standard built-in exceptions
...
The goal for Python 3.0 is to require any kundi raised as an exception to derive
kutoka :exc:`BaseException` or some descendant of :exc:`BaseException`, and future
releases in the Python 2.x series may begin to enforce this constraint.
Therefore, I suggest you begin making all your exception classes derive kutoka
...
.. seealso::
   ...
   :pep:`352` - Required Superkundi for Exceptions
...
* The :class:`dict` type has a new hook for letting subclasses provide a default
  ...
  to implement the new :class:`defaultdict` kundi in the :mod:`collections`
...
* Both 8-bit and Unicode strings have new ``partition(sep)``  and
  ...
  kutoka the end of the string; the ``r`` stands for 'reverse'.
...
* The :func:`min` and :func:`max` built-in functions gained a ``key`` keyword
  ...
  smallest/largest return value kutoka this function. For example, to find the
...
* One error that Python programmers sometimes make is forgetting to include an
  ...
  triggered when an agiza would have picked up a directory as a package but no
...
* The list of base classes in a kundi definition can now be empty.   As an
...
held in Reykjavik, Iceland, kutoka May 21--28 2006. The sprint focused on speed
enhancements to the CPython implementation and was funded by EWT LLC with local
support kutoka CCP Games.  Those optimizations added at this sprint are specially
...
* It's now illegal to mix iterating over a file  with ``for line in file`` and
  ...
  methods will now trigger a :exc:`ValueError` kutoka the :meth:`read\*` method.
...
* The :mod:`csv` module, which parses files in comma-separated value format,
  ...
  argument will return the currently-set limit.  The :class:`reader` kundi now has
  a :attr:`line_num` attribute that counts the number of physical lines read kutoka
...
* The :class:`~datetime.datetime` kundi in the :mod:`datetime`  module now has a
...
* The :mod:`doctest` module gained a ``SKIP`` option that keeps an example kutoka
  ...
  :class:`DocFileSuite` kundi to specify the file's encoding.  This makes it
...
* The :mod:`mailbox` module underwent a massive rewrite to add the capability to
  ...
  one::
     ...
     # 'factory=None' uses email.Message.Message as the kundi representing
     # individual messages.
...
* The :mod:`optparse` module was updated to version 1.5.1 of the Optik library.
  The :class:`OptionParser` kundi gained an :attr:`epilog` attribute, a string
...
* The :mod:`os` module underwent several changes.  The :attr:`stat_float_times`
  ...
  3-tuple of *process-id*, *exit-status*, *resource-usage* as returned kutoka the
...
* The :mod:`pickle` and :mod:`cPickle` modules no longer accept a return value
  of ``None`` kutoka the :meth:`__reduce__` method; the method must return a tuple
...
* The :mod:`pkgutil` module, containing various utility functions for finding
  packages, was enhanced to support :pep:`302`'s agiza hooks and now also works for
...
* The :class:`~queue.Queue` kundi provided by the :mod:`Queue` module gained two new
...
* The :mod:`socket` module now supports :const:`AF_NETLINK` sockets on Linux,
  thanks to a patch kutoka Philippe Biondi.   Netlink sockets are a Linux-specific
  ...
  ``recvkutoka_into(buffer)``, store the received data in an object  that
...
* The :mod:`struct` is now faster because it  compiles format strings into
  ...
  You can also pack and unpack data to and kutoka buffer objects directly using the
  ``pack_into(buffer, offset, v1, v2, ...)`` and ``unpack_kutoka(buffer,
...
* The Python developers switched kutoka CVS to Subversion during the 2.5
...
* The :class:`TarFile` kundi in the :mod:`tarfile` module now has an
  :meth:`extractall` method that extracts all members kutoka the archive into the
...
* New module: the  :mod:`uuid` module generates  universally unique identifiers
  ...
  versions that are generated kutoka a starting string, kutoka system properties, or
  purely randomly.  This module contains a :class:`UUID` kundi and  functions
...
* The :mod:`xmlrpclib` module now supports returning  :class:`~datetime.datetime` objects
  ...
  function or the :class:`Unmarshaller` kundi to enable this feature. (Contributed
  by Skip Montanaro.)
...
:class:`CDLL` kundi and provide the name or path of the shared library or DLL.
...
:mod:`ElementTree`, :mod:`ElementPath`, and :mod:`ElementInclude` kutoka
...
may use a different placeholder, such as ``%s`` or ``:1``.) For example::
   ...
   # Do this instead
   t = (symbol,)
   c.execute('select * kutoka stocks where symbol=?', t)
...
This example uses the iterator form::
   ...
   >>> c = conn.cursor()
   >>> c.execute('select * kutoka stocks order by price')
   >>> for row in c:
   ...    print row
   ...
...
up a server takes only a few lines of code::
   ...
   kutoka wsgiref agiza simple_server
...
* The Python source tree was converted kutoka CVS to Subversion,  in a complex
...
* The largest change to the C API came kutoka :pep:`353`, which modifies the
...
* The design of the bytecode compiler has changed a great deal,  no longer
  ...
  the  *flags* parameter::
     ...
     kutoka _ast agiza PyCF_ONLY_AST
     ast = compile("""a=0
     for i in range(10):
         a += i
  ...
  .. List of names taken kutoka Jeremy's python-dev post at
...
* Library: The :mod:`pickle` and :mod:`cPickle` modules no longer accept a
  return value of ``None`` kutoka the :meth:`__reduce__` method; the method must
