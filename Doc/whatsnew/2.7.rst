...
to Python 3, several new features kutoka the Python 3.x series have been
included in 2.7.
...
* As the Python 2.7 standard library ages, making effective use of the
  ...
  of new modules and features kutoka the Python 3 standard library that are
  ...
  installing software kutoka the Python Package Index.
...
For projects wishing to migrate kutoka Python 2 to Python 3, or for library
...
users kutoka seeing warnings triggered by an application.  This change
...
Much as Python 2.6 incorporated features kutoka Python 3.0,
...
the experiences kutoka those implementations, 2.7 introduces a new
:class:`~collections.OrderedDict` kundi in the :mod:`collections` module.
...
produce a dictionary kutoka different sources: construct one with code;
...
* The :class:`~logging.handlers.SysLogHandler` kundi now supports
...
* The :class:`~logging.LoggerAdapter` kundi gained an
...
.. XXX: Logger objects don't have a kundi declaration so the link don't work
...
* The syntax for set literals has been backported kutoka Python 3.x.
  Curly brackets are used to surround the contents of the resulting
  mutable set; set literals are
  distinguished kutoka dictionaries by not containing colons and values.
...
* Dictionary and set comprehensions are another feature backported kutoka
...
* The :keyword:`with` statement can now use multiple context managers
  in one statement.  Context managers are processed kutoka left to right
...
* Conversions kutoka long integers and regular integers to floating
...
* The :meth:`str.format` method now supports automatic numbering of the replacement
  ...
  The auto-numbering takes the fields kutoka left to right, so the first ``{...}``
...
* The :keyword:`import` statement will no longer try an absolute import
  if a relative agiza (e.g. ``kutoka .os agiza sep``) fails.  This
  fixes a bug, but could possibly break certain :keyword:`!import`
  statements that were only working by accident.  (Fixed by Meador Inge;
  :issue:`7902`.)

* It's now possible for a subkundi of the built-in :class:`unicode` type
...
* When using ``@classmethod`` and ``@staticmethod`` to wrap
  methods as kundi or static methods, the wrapper object now
...
* Extra parentheses in function definitions are illegal in Python 3.x,
  meaning that you get a syntax error kutoka ``def f((x)): pass``.  In
...
they occur, but turn warnings kutoka the :mod:`Cookie` module into an
...
* The garbage collector now performs better for one common usage
  ...
  number of survivor objects kutoka the middle generation exceeds 10% of
...
* Long integers are now stored internally either in base 2**15 or in base
  ...
  Apart kutoka the performance improvements this change should be
...
* The :mod:`pickle` and :mod:`cPickle` modules now automatically
  ...
  of the objects resulting kutoka unpickling.  (Contributed by Jake
  McGuire; :issue:`5084`.)
...
* The :mod:`bdb` module's base debugging kundi :class:`~bdb.Bdb`
  ...
  kutoka a module that matches one of these patterns.
...
* The :mod:`binascii` module now supports the buffer API, so it can be
  ...
  (Backported kutoka 3.x by Florent Xicluna; :issue:`7703`.)

* Updated module: the :mod:`bsddb` module has been updated kutoka 4.7.2devel9
  to version 4.8.4 of
...
* New class: the :class:`~collections.Counter` kundi in the :mod:`collections`
  ...
  The :class:`~collections.namedtuple` kundi now has an optional *rename* parameter.
  If *rename* is true, field names that are invalid because they've
  been repeated or aren't legal Python identifiers will be
  renamed to legal names that are derived kutoka the field's
  ...
  Finally, the :class:`~collections.Mapping` abstract base kundi now
...
* New method: the :class:`~decimal.Decimal` kundi gained a
  :meth:`~decimal.Decimal.kutoka_float` kundi method that performs an exact
  ...
  if any.
  For example, ``Decimal.kutoka_float(0.1)`` returns
  ...
  Most of the methods of the :class:`~decimal.Context` kundi now accept integers
...
* The :mod:`email` module's :class:`~email.message.Message` kundi will
...
* The :class:`~fractions.Fraction` kundi now accepts a single float or
...
* New kundi decorator: :func:`~functools.total_ordering` in the :mod:`functools`
  module takes a kundi that defines an :meth:`__eq__` method and one of
...
* The default :class:`~httplib.HTTPResponse` kundi used by the :mod:`httplib` module now
...
* The :mod:`ihooks` module now supports relative imports.  Note that
  ...
  (Relative agiza support added by Neil Schemenauer.)
...
* Updated module: The :mod:`io` library has been upgraded to the version shipped with
  ...
  One minor resulting change: the :class:`io.TextIOBase` kundi now
  has an :attr:`errors` attribute giving the error setting
  used for encoding and decoding errors (one of ``'strict'``, ``'replace'``,
  ``'ignore'``).

  The :class:`io.FileIO` kundi now raises an :exc:`OSError` when passed
...
* New function: ``itertools.compress(data, selectors)`` takes two
  ...
  returns all the possible *r*-length combinations of elements kutoka the
...
* The :mod:`mailbox` module's :class:`~mailbox.Maildir` kundi now records the
...
* New functions: the :mod:`os` module wraps the following POSIX system
  ...
  The :func:`os.fork` function now re-initializes the agiza lock in
  the child process; this fixes problems on Solaris when :func:`~os.fork`
  is called kutoka a thread.  (Fixed by Zsolt Cserna; :issue:`7242`.)
...
* New function: :func:`~runpy.run_path` in the :mod:`runpy` module
  ...
  if it doesn't, some other :file:`__main__.py` might be imported kutoka
...
* The :func:`~socket.create_connection` function
  ...
  The :meth:`~socket.socket.recv_into` and :meth:`~socket.socket.recvkutoka_into`
  methods will now write into objects that support the buffer API, most usefully
  the :class:`bytearray` and :class:`memoryview` objects.  (Implemented by
  Antoine Pitrou; :issue:`8104`.)

* The :mod:`SocketServer` module's :class:`~SocketServer.TCPServer` kundi now
  supports socket timeouts and disabling the Nagle algorithm.
  The :attr:`~SocketServer.TCPServer.disable_nagle_algorithm` kundi attribute
  defaults to ``False``; if overridden to be true,
  new request connections will have the TCP_NODELAY option set to
  prevent buffering many small sends into a single TCP packet.
  The :attr:`~SocketServer.BaseServer.timeout` kundi attribute can hold
...
* Updated module: the :mod:`sqlite3` module has been updated to
  ...
  the ability to load SQLite extensions kutoka shared libraries.
...
* The :mod:`ssl` module's :class:`~ssl.SSLSocket` objects now support the
  ...
  code being returned kutoka :meth:`recv` operations that trigger an SSL
...
* The :mod:`tarfile` module's default error handling has changed, to
  ...
  If the callable returns ``None``, the file will be excluded kutoka the
  resulting archive.  This is more powerful than the existing
  *exclude* argument, which has therefore been deprecated.
  (Added by Lars Gustäbel; :issue:`6856`.)
  The :class:`~tarfile.TarFile` kundi also now supports the context management protocol.
...
* The Unicode database provided by the :mod:`unicodedata` module is
  ...
  includes information kutoka the :file:`Unihan.txt` data file (patch
...
* New class: the :class:`~weakref.WeakSet` kundi in the :mod:`weakref`
...
agiza process.  Python 2.7 doesn't contain the complete
...
Here are some examples::
    ...
    >>> anydbm
    <module 'anydbm' kutoka '/p/python/Lib/anydbm.py'>
    >>> # Relative import
    >>> file_util = import_module('..file_util', 'distutils.command')
    >>> file_util
    <module 'distutils.file_util' kutoka '/python/Lib/distutils/file_util.pyc'>
...
platform name, and whether Python is running kutoka its source
directory.

Some of the functions in the module are:

* :func:`~sysconfig.get_config_var` returns variables kutoka Python's
...
* :func:`~sysconfig.is_python_build` returns true if you're running a
  binary kutoka a Python source tree, and false otherwise.
...
packaged as the :mod:`unittest2` package, kutoka
https://pypi.org/project/unittest2.

When used kutoka the command line, the module can automatically discover
...
:meth:`~unittest.TestCase.tearDownClass` methods that must be defined as kundi methods
...
:func:`~unittest.main` to be used kutoka the interactive interpreter.
...
* New :class:`~xml.etree.ElementTree.Element` method:
  :meth:`~xml.etree.ElementTree.Element.extend` appends the items kutoka a
  sequence to the element's children.  Elements themselves behave like
  sequences, so it's easy to move children kutoka one element to
  another::

    kutoka xml.etree agiza ElementTree as ET
...
* New function: stemming kutoka the rewrite of string-to-float conversion,
...
* New function: :c:func:`PySys_SetArgvEx` sets the value of
  ...
  :file:`os.py`) that your application would then agiza and run.
...
* The complicated interaction between threads and process forking has
  ...
  If other threads were holding a lock, such as Python's agiza lock,
  ...
  Python 2.7 acquires the agiza lock before performing an
  ...
  kutoka this clean-up.

  (Fixed by Thomas Wouters; :issue:`1590864`.)

* The :c:func:`Py_Finalize` function now calls the internal
  :func:`threading._shutdown` function; this prevents some exceptions kutoka
...
can agiza the module, access its dictionary to get the capsule
...
:c:type:`PyCObject` kutoka module A and somehow substituting it for the
...
* The :mod:`msvcrt` module now contains some constants kutoka
...
* The :mod:`mimelib` module will now read the MIME database kutoka
...
* The path ``/Library/Python/2.7/site-packages`` is now appended to
  ...
  (Changed by Ronald Oussoren; :issue:`4865`.)
   ...
   .. versionchanged:: 2.7.13
     ...
     ``sys.path`` for user-installed Pythons such as kutoka the python.org
...
* When importing a module kutoka a :file:`.pyc` or :file:`.pyo` file
...
* The string :meth:`format` method changed the default precision used
  for floating-point and complex numbers kutoka 6 decimal
...
* Because of an optimization for the :keyword:`with` statement, the special
  ...
  affects new-style classes (derived kutoka :class:`object`) and C extension
  types.  (:issue:`6101`.)
...
* :func:`hmac.compare_digest` was backported kutoka Python 3 to make a timing
...
* :func:`hashlib.pbkdf2_hmac` was backported kutoka Python 3 to make a hashing
...
* :data:`hashlib.algorithms_guaranteed` and
  :data:`hashlib.algorithms_available` were backported kutoka Python 3 to make
