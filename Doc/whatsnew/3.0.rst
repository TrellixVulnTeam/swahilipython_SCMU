...
* :func:`map` and :func:`filter` return iterators.  If you really need
  ...
  compatibility with :func:`map` kutoka Python 2.x, also wrap the sequences in
...
* As the :class:`str` and :class:`bytes` types cannot be mixed, you
  ...
  to go kutoka :class:`str` to :class:`bytes`, and :meth:`bytes.decode`
  to go kutoka :class:`bytes` to :class:`str`.  You can also use
...
* Filenames are passed to and returned kutoka APIs as (Unicode) strings.
...
* The :mod:`StringIO` and :mod:`cStringIO` modules are gone.  Instead,
  agiza the :mod:`io` module and use :class:`io.StringIO` or
...
* Keyword arguments are allowed after the list of base classes in a
  kundi definition.  This is used by the new convention for specifying
  a metakundi (see next section), but can be used for other purposes
  as well, as long as the metakundi supports it.
...
* :pep:`3109` and :pep:`3134`: new :keyword:`raise` statement syntax:
  :samp:`raise [{expr} [kutoka {expr}]]`.  See below.
...
* Change kutoka :keyword:`except` *exc*, *var* to
  :keyword:`!except` *exc* :keyword:`!as` *var*.  See :pep:`3110`.

* :pep:`3115`: New Metakundi Syntax.  Instead of::
  ...
  new-style classes without deriving every kundi kutoka
  :class:`object`.)
...
* The :keyword:`kutoka` *module* :keyword:`import` ``*`` syntax is only
  allowed at the module level, no longer inside functions.

* The only acceptable syntax for relative imports is :samp:`kutoka .[{module}]
  agiza {name}`.  All :keyword:`import` forms not starting with ``.`` are
...
Since many users presumably make the jump straight kutoka Python 2.5 to
...
* :ref:`pep-0343`.  The :keyword:`with` statement is now a standard
  feature and no longer needs to be imported kutoka the :mod:`__future__`.
...
* :ref:`pep-3105`.  This is now a standard feature and no longer needs
  to be imported kutoka :mod:`__future__`.  More details were given above.
...
* A common pattern in Python 2.x is to have one version of a module
  ...
  Users should always agiza the standard version, which attempts to
  agiza the accelerated version and falls back to the pure Python
  version.  The :mod:`pickle` / :mod:`cPickle` pair received this
  treatment.  The :mod:`profile` module is on the list for 3.1.  The
  :mod:`StringIO` module has been turned into a kundi in the :mod:`io`
  module.
...
* Cleanup of the :class:`array.array` type: the :meth:`read` and
  :meth:`write` methods are gone; use :meth:`kutokafile` and
...
* A new system for  built-in string formatting operations replaces the
  ...
  kutoka the language at some later time.)  Read :pep:`3101` for the full
  scoop.
...
* :pep:`352`: All exceptions must be derived (directly or indirectly)
  kutoka :exc:`BaseException`.  This is the root of the exception
  hierarchy.  This is not new as a recommendation, but the
  *requirement* to inherit kutoka :exc:`BaseException` is new.  (Python
...
* Almost all exceptions should actually derive kutoka :exc:`Exception`;
  :exc:`BaseException` should only be used as a base kundi for
...
* :pep:`3134`: Exception chaining.  There are two cases: implicit
  ...
  Explicit chaining is invoked with this syntax::
    ...
    raise SecondaryException() kutoka primary_exception
...
* The concept of "unbound methods" has been removed kutoka the language.
  When referencing a method as a kundi attribute, you now get a plain
  function object.
...
* :pep:`3135`: New :func:`super`.  You can now invoke :func:`super`
  ...
  defined inside a :keyword:`class` statement) the right kundi and
...
* :pep:`3111`: :func:`raw_input` was renamed to :func:`input`.  That
  is, the new :func:`input` function reads a line kutoka
...
* The :func:`round` function rounding strategy and return type have
  ...
  result instead of away kutoka zero.  (For example, ``round(2.5)`` now
...
* New C API :c:func:`PyImport_ImportModuleNoBlock`, works like
  :c:func:`PyImport_ImportModule` but won't block on the agiza lock
  (returning an error instead).

* Renamed the boolean conversion C-level slot and method:
  ``nb_nonzero`` is now ``nb_bool``.

* Removed :c:macro:`METH_OLDARGS` and :c:macro:`WITH_CYCLE_GC` kutoka the C API.
...
1. Port to Python 2.6.  This should be no more work than the average
   port kutoka Python 2.x to Python 2.(x+1).  Make sure all your tests
   pass.
