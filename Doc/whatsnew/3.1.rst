...
the experiences kutoka those implementations, a new
:class:`collections.OrderedDict` kundi has been introduced.
...
* Added a :class:`collections.Counter` kundi to support convenient
...
* Added a new module, :mod:`tkinter.ttk` for access to the Tk themed widget set.
  ...
  implementing a widget's behavior kutoka the code implementing its appearance.
...
* The :mod:`decimal` module now supports methods for creating a
  decimal object kutoka a binary :class:`float`.  The conversion is
  exact but can sometimes be surprising::

      >>> Decimal.kutoka_float(1.1)
...
* The :mod:`itertools` module grew two new functions.  The
  ...
  kutoka APL.  Also, the existing :func:`itertools.count` function now has
...
* The :mod:`logging` module now implements a simple :class:`logging.NullHandler`
  kundi for applications that are not using logging but are calling
...
* The :mod:`unittest` module now supports skipping individual tests or classes
  ...
  TestResult::
    ...
    kundi TestGizmo(unittest.TestCase):
...
* Integers are now stored internally either in base 2**15 or in base
  ...
  Apart kutoka the performance improvements this change should be invisible to
