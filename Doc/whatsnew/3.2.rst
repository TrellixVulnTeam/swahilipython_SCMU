...
.. seealso::
   ...
   :ref:`upgrading-optparse-code` for details on the differences kutoka :mod:`optparse`.
...
to create configurations kutoka JSON or YAML files, nor did they support
incremental configuration, which is needed for specifying logger options kutoka a
command line.
...
resources.  The kundi is a :term:`context manager` and can be used in a
...
launch of four parallel threads for copying files::
  ...
  agiza concurrent.futures, shutil
...
To solve this problem, Python's agiza machinery has been extended to use
...
"mymodule.unladen10.pyc".  And to prevent all of these new files kutoka
...
Aside kutoka the filenames and target directories, the new scheme has a few
...
* The tag that is unique to each interpreter is accessible kutoka the :mod:`imp`
  module:
...
* Scripts that try to deduce source filename kutoka the imported file now need to
  be smarter.  It is no longer sufficient to simply strip the "c" kutoka a ".pyc"
  filename.  Instead, use the new functions in the :mod:`imp` module:

  >>> imp.source_kutoka_cache('c:/py32/lib/__pycache__/collections.cpython-32.pyc')
  'c:/py32/lib/collections.py'
  >>> imp.cache_kutoka_source('c:/py32/lib/collections.py') # doctest: +SKIP
  'c:/py32/lib/__pycache__/collections.cpython-32.pyc'
...
In Python itself, the tags are accessible kutoka functions in the :mod:`sysconfig`
module::
...
For developers porting WSGI applications kutoka Python 2, here are the salient
points:
...
* If instead, the app encoded output headers or decoded input headers, then the
  ...
  encoded in utf-8 was using ``h.encode('utf-8')`` now needs to convert kutoka
...
:func:`wsgiref.handlers.read_environ` for transcoding CGI variables kutoka
...
* The interpreter can now be started with a quiet option, ``-q``, to prevent
  the copyright and version information kutoka being displayed in the interactive
...
* The :func:`hasattr` function works by calling :func:`getattr` and detecting
  ...
  would otherwise be absent kutoka the kundi dictionary.  Formerly, *hasattr*
...
* :class:`memoryview` objects now have a :meth:`~memoryview.release()` method
  ...
  release of any resources that were acquired when requesting a buffer kutoka the
  original object.
...
* A new warning category, :exc:`ResourceWarning`, has been added.  It is
  ...
  of enabling the warning kutoka the command line:
...
* The :func:`callable` builtin function kutoka Py2.x was resurrected.  It provides
...
* Python's agiza mechanism can now load modules installed in directories with
...
* New functions :func:`~email.message_kutoka_bytes` and
  :func:`~email.message_kutoka_binary_file`, and new classes
...
* A new kundi :class:`~email.generator.BytesGenerator` produces bytes as output,
...
* The :mod:`smtplib` :class:`~smtplib.SMTP` kundi now accepts a byte string
  ...
  *kutoka_addr* and *to_addrs* addresses directly kutoka the object.
...
* :func:`xml.etree.ElementTree.kutokastringlist` which builds an XML document
  kutoka a sequence of fragments
...
* The :mod:`functools` module includes a new decorator for caching function
  ...
  (Contributed by Raymond Hettinger and incorporating design ideas kutoka Jim
...
* To aid in porting programs kutoka Python 2, the :func:`functools.cmp_to_key`
...
* The :mod:`itertools` module has a new :func:`~itertools.accumulate` function
  ...
  >>> kutoka itertools agiza accumulate
  ...
  kutoka Mark Dickinson.)

collections
-----------

* The :class:`collections.Counter` kundi now has two forms of in-place
...
* The :class:`collections.OrderedDict` kundi has a new method
  ...
  kutoka the oldest to the most recently accessed.

  >>> kutoka collections agiza OrderedDict
  >>> d = OrderedDict.kutokakeys(['a', 'b', 'X', 'd', 'e'])
  >>> list(d)
  ['a', 'b', 'X', 'd', 'e']
  >>> d.move_to_end('X')
  >>> list(d)
...
* The :class:`collections.deque` kundi grew two new methods
...
synchronization kundi for making multiple threads wait until all of them have
...
Example of using barriers::
    ...
    kutoka threading agiza Barrier, Thread
...
* The :meth:`datetime.date.strftime` method is no longer restricted to years
  after 1900.  The new supported year range is kutoka 1000 to 9999 inclusive.
...
.. doctest::
   ...
   >>> kutoka math agiza erf, erfc, sqrt
...
>>> kutoka math agiza gamma, lgamma
...
implemented::
    ...
    kundi Temperature(metaclass=abc.ABCMeta):
        @abc.abstractclassmethod
        def kutoka_fahrenheit(cls, t):
            ...
        @abc.abstractclassmethod
        def kutoka_celsius(cls, t):
            ...
...
:class:`string.Template`.  The following three configurations are equivalent::
    ...
    >>> kutoka logging agiza basicConfig
...
* The :class:`decimal.Decimal` constructor now accepts :class:`float` objects
  directly so there in no longer a need to use the :meth:`~decimal.Decimal.kutoka_float`
  method (:issue:`8257`).
...
:meth:`~fractions.Fraction.kutoka_float()` and :meth:`~fractions.Fraction.kutoka_decimal`
...
The :class:`ftplib.FTP` kundi now supports the context management protocol to
...
The :class:`~ftplib.FTP_TLS` kundi now accepts a *context* parameter, which is a
...
Also, the :class:`zipfile.ZipExtFile` kundi was reworked internally to represent
...
The :class:`~tarfile.TarFile` kundi can now be used as a context manager.  In
...
* The :meth:`sqlite3.Connection.enable_load_extension` and
  ...
  extensions kutoka ".so" files.  One well-known extension is the fulltext-search
...
:func:`~html.escape`, which is used for escaping reserved characters kutoka HTML
markup:
...
* A new class, :class:`~ssl.SSLContext`, serves as a container for persistent
  ...
  an SSL socket kutoka an SSL context.
...
* A new function, :func:`ssl.match_hostname`, supports server identity
  ...
  (kutoka :rfc:`2818`) which are also suitable for other protocols.
...
* The command-line call ``python -m unittest`` can now accept file paths
  ...
  kutoka the top-level directory.  The top-level directory can be specified with
...
* Experimentation at the interactive prompt is now easier because the
  :class:`unittest.case.TestCase` kundi can now be instantiated without
  arguments:
...
Now, multiple selections are made kutoka a range up to the next power of two and a
...
:class:`~poplib.POP3_SSL` kundi now accepts a *context* parameter, which is a
:class:`ssl.SSLContext` object allowing bundling SSL configuration options,
...
* The :mod:`inspect` module has a new function
  ...
  generator-iterator::
    ...
    >>> kutoka inspect agiza getgeneratorstate
    >>> def gen():
    ...     yield 'demo'
    >>> g = gen()
    >>> getgeneratorstate(g)
...
object.  The former returns a string and the latter prints it::
    ...
    Constants:
       0: 'Choose a random element kutoka a non-empty sequence.'
       1: 'Cannot choose kutoka an empty sequence'
    Names:
       0: _randbelow
       1: len
       2: ValueError
...
Conveniently, some of site's functionality is accessible directly kutoka the
command-line:
...
* The :class:`Pdb` kundi constructor now accepts a *nosigint* argument.
...
:class:`ConfigParser` kundi was removed in favor of :class:`SafeConfigParser`
...
reading directly kutoka dictionaries and strings.
...
is not very useful.  Instead, it is best to extract messages kutoka
a :class:`~mailbox.Message` object or to load them kutoka binary input.

(Contributed by R. David Murray, with efforts kutoka Steffen Daode Nurpmeso and an
initial patch by Victor Stinner in :issue:`9124`.)

turtledemo
----------

The demonstration code for the :mod:`turtle` module was moved kutoka the *Demo*
...
(Moved kutoka the Demo directory by Alexander Belopolsky in :issue:`10199`.)
...
* The mechanism for serializing execution of concurrently running Python threads
  ...
  Additional details about the implementation can be read kutoka a `python-dev
...
* Recursive locks (created with the :func:`threading.RLock` API) now benefit
  kutoka a C implementation which makes them as fast as regular locks, and between
...
* The C functions that access the Unicode Database now accept and return
  characters kutoka the full Unicode range, even on narrow unicode builds
...
* Removed the ``O?`` format kutoka the *PyArg_Parse* functions.  The format is no
...
* The :mod:`configparser` module has a number of clean-ups.  The major change is
  to replace the old :class:`ConfigParser` kundi with long-standing preferred
  ...
  * Duplicate sections or options kutoka a single source now raise either
  ...
  * Comments now can be indented.  Consequently, for **;** or **#** to appear at
    ...
    keeps comment prefix characters in values kutoka being mistaken as comments.
...
* The :meth:`array.tostring` and :meth:`array.kutokastring` have been renamed to
  :meth:`array.tobytes` and :meth:`array.kutokabytes` for clarity.  The old names
...
* The :class:`xml.etree.ElementTree` kundi now raises an
...
* Support for legacy HTTP 0.9 has been removed kutoka :mod:`urllib.request`
