...
* New ``yield kutoka`` expression for :ref:`generator delegation <pep-380>`.
...
* Rewritten :ref:`agiza machinery <importlib>` based on :mod:`importlib`.
...
libraries), and are optionally segregated kutoka the system-wide site
packages.  Their concept and implementation are inspired by the popular
``virtualenv`` third-party package, but benefit kutoka tighter integration
...
* Python now always supports the full range of Unicode code points, including
  non-BMP ones (i.e. kutoka ``U+0000`` to ``U+10FFFF``).  The distinction between
...
The launcher can also be used explicitly kutoka the command line as the ``py``
...
constant kutoka the :mod:`errno` module, you can catch the adequate
...
avoided.  For example, the following code written for Python 3.2::
    ...
    kutoka errno agiza ENOENT, EACCES, EPERM
...
can now be written without the :mod:`errno` agiza and without manual
...
.. index::
   single: yield; yield kutoka (in What's New)
...
PEP 380 adds the ``yield kutoka`` expression, allowing a :term:`generator` to
delegate
...
kutoka`` expression actually allows delegation to arbitrary subiterators.

For simple iterators, ``yield kutoka iterable`` is essentially just a shortened
form of ``for item in iterable: yield item``::

    >>> def g(x):
    ...     yield kutoka range(x, 0, -1)
    ...     yield kutoka range(x)
    ...
    >>> list(g(5))
    [5, 4, 3, 2, 1, 0, 1, 2, 3, 4]

However, unlike an ordinary loop, ``yield kutoka`` allows subgenerators to
receive sent and thrown values directly kutoka the calling scope, and
...
applications that convert between exception types::
    ...
    ...         except KeyError:
    ...             raise AttributeError(attr) kutoka None
    ...
    >>> D({}).x
...
Without the ``kutoka None`` suffix to suppress the cause, the original
...
To ease the transition kutoka Python 2 for Unicode aware Python applications
...
Functions and kundi objects have a new ``__qualname__`` attribute representing
the "path" kutoka the module top-level to their definition.  For global functions
...
how they might be accessible kutoka the global scope.
...
``importlib`` for its built-in agiza system may use ``cache_tag`` to control
...
:issue:`14605` - Make agiza machinery explicit
...
machinery powering agiza to be exposed instead of being implicit and hidden
...
agiza semantics. And finally it eases the maintenance of import, allowing for
future growth to occur.

For the common user, there should be no visible change in semantics.  For
those whose code currently manipulates agiza or calls import
...
making the agiza statement work. That means the various importers that were
...
this kundi was an implicit member of :attr:`sys.path_hooks`.

For loaders, the new abstract base kundi :class:`importlib.abc.FileLoader` helps
...
the C code of agiza instead of being directly exposed. This means that one can
...
attribute. Until such time, though, agiza is setting the module post-load.

Loaders are also now expected to set the ``__package__`` attribute kutoka
:pep:`366`. Once again, agiza itself is already setting this on all loaders
kutoka :mod:`importlib` and agiza itself is setting the attribute post-load.
...
Previous versions of CPython have always relied on a global agiza lock.
...
serializes importation of a given module kutoka multiple threads (preventing
...
it can now be used as a kundi decorator (:issue:`10868`).
...
The :mod:`bz2` module has been rewritten kutoka scratch. In the process, several
new features have been added:

* New :func:`bz2.open` function: open a bzip2-compressed file in binary or
  text mode.

* :class:`bz2.BZ2File` can now read kutoka and write to arbitrary file-like
...
Addition of a new :class:`~collections.ChainMap` kundi to allow treating a
...
The :class:`~collections.Counter` kundi now supports the unary ``+`` and ``-``
...
``__enter__`` method (for example, synchronisation objects kutoka the
 ...
 * The :class:`curses.window` kundi has a new :meth:`~curses.window.get_wch`
...
Performance gains range kutoka 10x for database applications to 100x for
...
policies.  When one of the new policies is used, any header retrieved kutoka
...
You can also create addresses kutoka parts::
...
Decoding to unicode is done automatically::
    ...
    >>> m2 = message_kutoka_string(str(m))
    >>> m2['to']
    'Éric <foo@example.com>'
...
package transparently encodes and decodes the unicode to and kutoka the RFC
...
New utility functions:
   ...
   * :func:`~email.utils.parsedate_to_datetime`: given a date string kutoka
...
* The :class:`~ftplib.FTP_TLS` kundi now provides a new
...
reports the current binding of all names referenced kutoka the function body and
...
The constructor of the :class:`~io.TextIOWrapper` kundi has a new
...
A kundi level attribute :attr:`~logging.handlers.SysLogHandler.append_nul` has
...
bytes kutoka the current file position to the end of the mapping.  (Contributed
...
to override the default behavior of inheriting the ``daemon`` flag kutoka
...
The :class:`nntplib.NNTP` kundi now supports the context management protocol to
...
* The :mod:`os` module has a new :func:`~os.sendfile` function which provides
  an efficient "zero-copy" way for copying data kutoka one file (or socket)
  descriptor to another. The phrase "zero-copy" refers to the fact that all of
  the copying of data between the two descriptors is done entirely by the
  kernel, with no copying of data into userspace buffers. :func:`~os.sendfile`
  can be used to efficiently copy data kutoka a file on disk to a network socket,
...
* Additional new  posix functions:
  ...
  * :func:`~os.pread`: Read kutoka a file descriptor at an offset, the file
    offset remains unchanged.
  * :func:`~os.pwrite`: Write to a file descriptor kutoka an offset, leaving
    the file offset unchanged.
  * :func:`~os.readv`: Read kutoka a file descriptor into a number of writable buffers.
...
* :func:`~os.times` and :func:`~os.uname`: Return type changed kutoka a tuple to
...
The Tk GUI and the :func:`~pydoc.serve` function have been removed kutoka the
...
* :class:`~sched.scheduler` kundi can now be safely used in multi-threaded
...
Solaris and derivative platforms have a new kundi :class:`select.devpoll`
...
The previously undocumented helper function ``quote`` kutoka the
...
* The :class:`~socket.socket` kundi now exposes additional methods to process
...
* The :class:`~socket.socket` kundi now supports the PF_CAN protocol family
  (https://en.wikipedia.org/wiki/Socketcan), on Linux
  (https://lwn.net/Articles/253425).

  (Contributed by Matthias Fuchs, updated by Tiago Gonçalves in :issue:`10141`.)

* The :class:`~socket.socket` kundi now supports the PF_RDS protocol family
  (https://en.wikipedia.org/wiki/Reliable_Datagram_Sockets and
  https://oss.oracle.com/projects/rds/).

* The :class:`~socket.socket` kundi now supports the ``PF_SYSTEM`` protocol
...
kundi instance, are now classes and may be subclassed.  (Contributed by Éric
...
value kutoka the parent thread (:issue:`6064`).
...
and the generic launchers :program:`xdg-open`, kutoka the FreeDesktop.org
...
* The default finders used by agiza now utilize a cache of what is contained
...
* The *index* argument to :func:`__import__` now defaults to 0 instead of -1
  ...
  perform a relative agiza followed by an absolute import, then perform the
  relative agiza using an index of 1, followed by another agiza using an
...
* :mod:`pkgutil` has been converted to use :mod:`importlib` internally. This
  ...
  emulation failed to match the behaviour of the real agiza system. The
  agiza emulation itself is still present, but is now deprecated. The
  :func:`pkgutil.iter_importers` and :func:`pkgutil.walk_packages` functions
  special case the standard agiza hooks so they are still supported even
...
* The deprecated ``Context._clamp`` attribute has been removed kutoka the
  :mod:`decimal` module.  It was previously replaced by the public attribute
  :attr:`~decimal.Context.clamp`.  (See :issue:`8540`.)

* The undocumented internal helper kundi ``SSLFakeFile`` has been removed
  kutoka :mod:`smtplib`, since its functionality has long been provided directly
...
* Code that used to work around the fact that the :mod:`threading` module used
  ...
  subkundi the now-public classes.
...
* When :program:`python` is started with :option:`-S`, ``agiza site``
