...
.. Rules for maintenance:
   ...
   * While the maintainer will periodically go through Misc/NEWS
   and add changes, it's best not to rely on this. We know kutoka experience
   ...
   for years, even if they're added later. We also know kutoka experience that
...
* :ref:`Retrieving certificates kutoka the Windows system cert store support
  <whatsnew34-win-cert-store>` for :mod:`ssl`.
* :ref:`Server-side SNI (Server Name Indication) support
  <whatsnew34-sni>` for :mod:`ssl`.
* The :class:`ssl.SSLContext` kundi has a :ref:`lot of improvements
...
automatic ``PATH`` modifications to have ``pip`` available kutoka the command
...
to and kutoka its hexadecimal representation (for example) can now be written
as::

    >>> kutoka codecs agiza encode, decode
    >>> encode(b"hello", "hex")
    b'68656c6c6f'
...
agiza machinery will use to load it (that is, a module specification).  This
helps simplify both the agiza implementation and several import-related APIs.
...
The public-facing changes kutoka the PEP are entirely backward-compatible.
...
* Frame objects now have a :func:`~frame.clear` method that clears all
  references to local variables kutoka the frame.  (Contributed by Antoine Pitrou
  in :issue:`17934`.)
...
without I/O, and *concrete paths*, which inherit kutoka pure paths but also
...
New kundi :class:`~abc.ABC` has :class:`~abc.ABCMeta` as its meta class.
Using ``ABC`` as a base kundi has essentially the same effect as specifying
...
The :class:`~argparse.FileType` kundi now accepts *encoding* and
...
encode and decode binary data kutoka and to ``Ascii85`` and the git/mercurial
...
intent of code that deliberately suppresses exceptions kutoka a single
...
kutoka a function that was written to implement a command line interface.
...
object in ways different kutoka those provided by the :mod:`~dis` module
itself.  For example::
...
In addition, a new application-friendly kundi :class:`~dis.Bytecode` provides
...
:class:`~dis.Bytecode` also has a kundi method,
:meth:`~dis.Bytecode.kutoka_traceback`, that provides the ability to manipulate a
traceback (that is, ``print(Bytecode.kutoka_traceback(tb).dis())`` is equivalent
to ``distb(tb)``).
...
argument, but does accept the *unixkutoka* and *policy* arguments. The
...
make extracting content kutoka and inserting content into email messages much
...
to behave like normal instance methods when included in a kundi definition.
...
:const:`NotImplemented` kutoka the underlying comparison function.  (Contributed
...
agiza by other programs, it gets improvements with every release. See
...
kutoka the IDLE :menuselection:`Help --> About IDLE` dialog.
...
The :func:`~importlib.reload` function has been moved kutoka :mod:`imp` to
...
New :mod:`importlib.util` functions :func:`~importlib.util.cache_kutoka_source`
and :func:`~importlib.util.source_kutoka_cache` replace the same-named functions
...
that decodes source kutoka bytes using universal newline processing.  This is
...
``__dir__`` methods and dynamic kundi attributes provided through
...
:class:`configparser.RawConfigParser` subkundi instance for the *fname*
...
Logging configuration data received kutoka a socket via the
...
memory a module occupies in memory when it is loaded kutoka a ``.pyc`` (or
...
inherit unneeded handles/file descriptors kutoka their parents (part of
:issue:`8713`).
...
``yield kutoka`` in a more useful fashion.  This is especially helpful when
debugging :mod:`asyncio` based programs.  (Contributed by Andrew Svetlov and
Xavier de Gaye in :issue:`16596`.)

The ``print`` command has been removed kutoka :mod:`pdb`, restoring access to the
Python :func:`print` function kutoka the pdb command line.  Python2's ``pdb`` did
...
The :mod:`pprint` module's :class:`~pprint.PrettyPrinter` kundi and its
...
:func:`pty.spawn` now returns the status value kutoka :func:`os.waitpid` on
...
explicit about the goal of the match, which avoids a kundi of subtle bugs where
...
:exc:`~smtplib.SMTPException` is now a subkundi of :exc:`OSError`, which allows
...
loads a set of default "certificate authority" (CA) certificates kutoka default
...
certificate information, and CRLs kutoka the Windows cert store.  (Contributed
...
:mod:`usercustomize` by deleting this attribute kutoka :mod:`sys` (or setting it
...
The :class:`~textwrap.TextWrapper` kundi has two new attributes/constructor
...
obtained kutoka the new :func:`~threading.main_thread` function.  In normal
conditions this will be the thread kutoka which the Python interpreter was
...
attributes on the kundi with the same name (see :mod:`Enum` for an example).
...
kundi attribute on the subclass.  (Contributed by Jason R Coombs in
:issue:`18978`.)
...
kutoka scratch.  There is also a new
:meth:`~urllib.request.Request.remove_header` method that can be used to remove
headers kutoka a :class:`~urllib.request.Request`.  (Contributed by Alexey
...
The :class:`~unittest.TestCase` kundi has a new method,
...
behavior by creating a :class:`~unittest.TestSuite` subkundi that defines a
...
:mod:`logging` module.  By default the message can come kutoka any logger and
...
New :class:`~weakref.WeakMethod` kundi simulates weak references to bound
methods.  (Contributed by Antoine Pitrou in :issue:`14631`.)

New :class:`~weakref.finalize` kundi makes it possible to register a callback
...
:class:`~zipfile.PyZipFile` kundi has a new *filterfunc* option that can be
used to control which directories and files are added to the archive.  For
example, this could be used to exclude test files kutoka the archive.
...
whole kundi of interpreter-shutdown-time errors, usually involving
...
* The new :c:func:`PyType_GetSlot` function has been added to the stable ABI,
  allowing retrieval of function pointers kutoka named type slots when using
...
* The :ref:`python <using-on-cmdline>` command has a new :ref:`option
  ...
  isolate the execution of a script kutoka the user's environment.  This is
...
* Tab-completion is now enabled by default in the interactive interpreter
  ...
  and is written to (and read kutoka) the file :file:`~/.python-history`.
...
* The :mod:`stat` module is now implemented in C, which means it gets the
  values for its constants kutoka the C header files, instead of having the
  values hard-coded in the python module as was previously the case.

* Loading multiple python modules kutoka a single OS module (``.so``, ``.dll``)
...
* A new opcode, :opcode:`LOAD_CLASSDEREF`, has been added to fix a bug in the
  loading of free variables in kundi bodies that could be triggered by certain
...
* :func:`os.urandom` now uses a lazily-opened persistent file descriptor
  so as to avoid using many file descriptors when run in parallel kutoka
...
* As mentioned in :ref:`whatsnew-pep-451`, a number of :mod:`importlib`
  ...
  :meth:`importlib.abc.SourceLoader.exec_module`) and let the agiza system
  ...
  functions are now handled automatically by the agiza system.
...
* The internal ``Netrc`` kundi in the :mod:`ftplib` module has been documented
...
Support for the following operating systems has been removed kutoka the source
and build tools:
...
* ``PyLoader`` and ``PyPycLoader`` have been removed kutoka :mod:`importlib`.
...
* Support for loading the deprecated ``TYPE_INT64`` has been removed kutoka
...
* The unused and undocumented internal ``Scanner`` kundi has been removed kutoka
...
* Frozen packages no longer set ``__path__`` to a list containing the package
  ...
  the agiza system to do the wrong thing on submodule imports if there was
...
* Frozen modules no longer define a ``__file__`` attribute. It's semantically
  incorrect for frozen modules to set the attribute as they are not loaded kutoka
  any explicit location. If you must know that a module comes kutoka frozen code
  then you can see if the module's ``__spec__.location`` is set to ``'frozen'``,
  check if the loader is a subkundi of
  :class:`importlib.machinery.FrozenImporter`,
  or if Python 2 compatibility is necessary you can use :func:`imp.is_frozen`.

* :func:`py_compile.compile` now raises :exc:`FileExistsError` if the file path
  it would write to is a symlink or a non-regular file. This is to act as a
  warning that agiza will overwrite those files with a regular file regardless
...
* Strings between ``kutoka __future__ agiza ...`` statements now *always* raise
...
* The :attr:`~cgi.FieldStorage.file` attribute is now automatically closed when
  ...
  were pulling the file object out separately kutoka the :class:`cgi.FieldStorage`
...
* Because :mod:`unittest.TestSuite` now drops references to tests after they
  ...
  creating a :mod:`~unittest.TestSuite` subkundi that defines a
...
* The :mod:`collections.abc` module has been slightly refactored as
  ...
  implicit import, you will need to add an explicit ``agiza collections.abc``
  (:issue:`20784`).
