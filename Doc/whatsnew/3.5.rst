...
* Generators have a new ``gi_yieldkutoka`` attribute, which returns the
  object being iterated by ``yield kutoka`` expressions. (Contributed
...
* The :mod:`ssl` module gained
  ...
  protocol handling kutoka network IO.
...
work needed to make up for this missing functionality detracts kutoka the
...
* methods of the :class:`~socket.socket` class: :meth:`~socket.socket.accept`,
  ...
  :meth:`~socket.socket.recv`, :meth:`~socket.socket.recvkutoka`,
...
combination with the ``yield kutoka`` construct.

This is a backwards incompatible change, so to enable the new behavior,
a :term:`__future__` agiza is necessary::
...
:option:`-O` or :option:`-OO`. Consequently, bytecode files generated kutoka
:option:`-O`, and :option:`-OO` may now exist simultaneously.
:func:`importlib.util.cache_kutoka_source` has an updated API to help with
this change.
...
This change brings the agiza semantics of extension modules that opt-in to
...
The :class:`~argparse.ArgumentParser` kundi now allows disabling
...
* The ``JoinableQueue`` kundi was removed, in favor of the
...
The :class:`~cgi.FieldStorage` kundi now supports the :term:`context manager`
...
The :class:`~collections.OrderedDict` kundi is now implemented in C, which
...
The :class:`~collections.deque` kundi now defines
...
The :class:`~collections.UserString` kundi now implements the
...
The default charset of HTML document changed kutoka ``"ISO-8859-1"``
to ``"utf-8"``.
(Contributed by Berker Peksag in :issue:`2052`.)

The :func:`~difflib.diff_bytes` function can now compare lists of byte
strings.  This fixes a regression kutoka Python 2.
...
A new policy option :attr:`Policy.mangle_kutoka_ <email.policy.Policy.mangle_kutoka_>`
...
The :class:`~imaplib.IMAP4` kundi now supports the :term:`context manager` protocol.
...
The :class:`util.LazyLoader <importlib.util.LazyLoader>` kundi allows for
...
object with code compiled kutoka a string by running
``exec(code, module.__dict__)``.
(Contributed by Brett Cannon in :issue:`21156`.)

The new :func:`util.module_kutoka_spec() <importlib.util.module_kutoka_spec>`
...
A new kundi method
:meth:`Signature.kutoka_callable() <inspect.Signature.kutoka_callable>` makes
...
network objects kutoka existing addresses::
...
The :class:`handlers.HTTPHandler <logging.handlers.HTTPHandler>` kundi now
...
The :class:`handlers.QueueListener <logging.handlers.QueueListener>` kundi now
...
A new :meth:`Path.home() <pathlib.Path.home>` kundi method can be used to get
...
The :class:`~smtpd.SMTPServer` kundi now advertises the ``8BITMIME`` extension
...
The :class:`~smtpd.SMTPServer` kundi now also supports the ``SMTPUTF8``
...
function on UNIX, resulting in uploads being kutoka 2 to 3 times faster than when
...
The new :class:`~ssl.SSLObject` kundi has been added to provide SSL protocol
...
kundi can be used to pass data between Python and an SSL protocol instance.
...
The :class:`~ssl.SSLSocket` kundi now implements
...
kundi no longer reset the socket timeout every time bytes are received or sent.
...
The :class:`~sqlite3.Row` kundi now fully supports the sequence protocol,
...
directories will be owned by the numeric ``uid`` and ``gid`` kutoka the tarfile.
...
The :mod:`unicodedata` module now uses data kutoka `Unicode 8.0.0
...
* The kundi constructor has a new *unsafe* parameter, which causes mock
...
The :class:`~unittest.mock.MagicMock` kundi now supports :meth:`__truediv__`,
...
kundi allows HTTP Basic Authentication credentials to be managed so as to
...
kundi constructor is now optional.
...
The :class:`client.ServerProxy <xmlrpc.client.ServerProxy>` kundi now supports
...
function, which exposes file information kutoka the underlying ``readdir`` or
``FindFirstFile``/``FindNextFile`` system calls.  (Contributed by
Ben Hoyt with help kutoka Victor Stinner in :issue:`23605`.)
...
The speed up can range kutoka 3 to 15 times.
...
Objects kutoka the :mod:`random` module now use 50% less memory on 64-bit
...
some platforms (the tag is optional, and CPython will agiza extensions without
...
The previously undocumented ``kutoka_function`` and ``kutoka_builtin`` methods of
:class:`inspect.Signature` are deprecated.  Use the new
:meth:`Signature.kutoka_callable() <inspect.Signature.kutoka_callable>`
...
* The ``__version__`` attribute has been dropped kutoka the email package.  The
  email code hasn't been shipped separately kutoka the stdlib for a long time,
  and the ``__version__`` string was not updated in the last few releases.

* The internal ``Netrc`` kundi in the :mod:`ftplib` module was deprecated in
  3.4, and has now been removed.
  (Contributed by Matt Chaput in :issue:`6623`.)

* The concept of ``.pyo`` files has been removed.

* The JoinableQueue kundi in the provisional :mod:`asyncio` module was
...
* The ``__name__`` attribute of generators is now set kutoka the function name,
  instead of being set kutoka the code name. Use ``gen.gi_code.co_name`` to
...
* HTTP status code and messages kutoka :mod:`http.client` and :mod:`http.server`
...
* When an agiza loader defines :meth:`importlib.machinery.Loader.exec_module`
  ...
  inherits kutoka :class:`importlib.abc.Loader` then there is nothing to do, else
...
* :pep:`488` has removed ``.pyo`` files kutoka Python and introduced the optional
  ``opt-`` tag in ``.pyc`` file names. The
  :func:`importlib.util.cache_kutoka_source` has gained an *optimization*
  ...
  *debug_override* parameter of the function is now deprecated. `.pyo` files
  ...
  in Python 3.5, all old `.pyo` files kutoka previous versions of Python are
...
* The :func:`inspect.getdoc` function now returns documentation strings
  inherited kutoka base classes.  Documentation strings no longer need to be
...
* Nested :func:`functools.partial` calls are now flattened.  If you were
  ...
  :func:`functools.partial` object or you can create a subkundi of
