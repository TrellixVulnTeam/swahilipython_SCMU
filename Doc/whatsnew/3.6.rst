...
* Customization of kundi creation has been simplified with the
  :ref:`new protocol <whatsnew36-pep487>`.

* The kundi attribute definition order is
...
the types of variables including kundi variables and instance variables::
...
``__annotations__`` attribute of a kundi or module.
...
possible to define *asynchronous generators*::
    ...
    async def ticker(delay, to):
        """Yield numbers kutoka 0 to *to* every *delay* seconds."""
        for i in range(to):
            yield i
...
PEP 487: Simpler customization of kundi creation
------------------------------------------------

It is now possible to customize subkundi creation without using a metaclass.
The new ``__init_subclass__`` classmethod will be called on the base class
whenever a new subkundi is created::
    ...
    kundi Plugin1(PluginBase):
        pass

    kundi Plugin2(PluginBase):
        pass

In order to allow zero-argument :func:`super` calls to work correctly kutoka
...
:meth:`~object.__set_name__` method.  Whenever a new kundi is defined, the new
method will be called on all descriptors included in the definition, providing
them with a reference to the kundi being defined and the name given to the
descriptor within the kundi namespace.  In other words, instances of
...
paths like :mod:`pathlib` kutoka working with pre-existing code,
...
Attributes in a kundi definition body have a natural ordering: the same
order in which the names appear in the source.  This order is now
preserved in the new class's :attr:`~object.__dict__` attribute.

Also, the effective default kundi *execution* namespace (returned kutoka
...
* It is now possible to set a :ref:`special method <specialnames>` to
  ...
  For example, if a kundi sets :meth:`__iter__` to ``None``, the class
  is not iterable.
...
* Import now raises the new exception :exc:`ModuleNotFoundError`
  (subkundi of :exc:`ImportError`) when it cannot find a module.  Code
...
* Class methods relying on zero-argument ``super()`` will now work correctly
  when called kutoka metakundi methods during kundi creation.
...
* The :func:`~asyncio.get_event_loop` function has been changed to
  always return the currently running loop when called kutoka coroutines
  and callbacks.
...
* New :func:`~asyncio.run_coroutine_threadsafe` function to submit
  coroutines to event loops kutoka other threads.
...
* New :meth:`StreamReader.readuntil() <asyncio.StreamReader.readuntil>`
  method to read data kutoka the stream until a separator bytes
  sequence appears.
...
kundi constructor now accepts an optional *thread_name_prefix* argument
...
The :class:`contextlib.AbstractContextManager` kundi has been added to
provide an abstract base kundi for context managers.  It provides a
sensible default implementation for `__enter__()` which returns
``self`` and leaves `__exit__()` an abstract method.  A matching
kundi has been added to the :mod:`typing` module as
...
The ``default_format`` attribute has been removed kutoka
...
:attr:`~email.policy.Policy.message_factory`, that controls what kundi is used
...
:class:`~enum.IntFlags` kundi for their constants.
...
as normal features.  Their settings have been moved kutoka the Extensions tab
...
:func:`importlib.util.cache_kutoka_source`,
:func:`importlib.util.source_kutoka_cache`, and
:func:`importlib.util.spec_kutoka_file_location` now accept a
...
To reduce code churn when upgrading kutoka Python 2.7 and the legacy
...
The :class:`~pdb.Pdb` kundi constructor has a new optional *readrc* argument
...
specified size kutoka the given population with optional weights.
...
(Contributed by Christian Heimes in :issue:`27744` with support kutoka
Victor Stinner.)
...
3DES has been removed kutoka the default cipher suites and ChaCha20 Poly1305
...
SSL session can be copied kutoka one client-side connection to another
...
The new :class:`~tracemalloc.DomainFilter` filter kundi has been added
...
The :class:`typing.ContextManager` kundi has been added for
representing :class:`contextlib.AbstractContextManager`.
(Contributed by Brett Cannon in :issue:`25609`.)

The :class:`typing.Collection` kundi has been added for
...
mark kundi variables.  As introduced in :pep:`526`, a variable annotation
wrapped in ClassVar indicates that a given attribute is intended to be used as
a kundi variable and should not be set on instances of that class.
...
The :mod:`unicodedata` module now uses data kutoka `Unicode 9.0.0
...
The :class:`~unittest.mock.Mock` kundi has the following improvements:
...
A new :meth:`ZipInfo.kutoka_file() <zipfile.ZipInfo.kutoka_file>` kundi method
allows making a :class:`~zipfile.ZipInfo` instance kutoka a filesystem file.
...
* The Python interpreter now uses a 16-bit wordcode instead of bytecode which
  ...
  (Contributed by Demur Rumed with input and reviews kutoka
  Serhiy Storchaka and Victor Stinner in :issue:`26647` and :issue:`28050`.)

* The :class:`asyncio.Future` kundi now has an optimized C implementation.
  (Contributed by Yury Selivanov and INADA Naoki in :issue:`26081`.)

* The :class:`asyncio.Task` kundi now has an optimized
...
* Optimize :meth:`bytes.kutokahex` and :meth:`bytearray.kutokahex`: they are now
...
* Creation of :class:`fractions.Fraction` instances kutoka floats and
...
``__path__`` kutoka the calling module when ``__spec__`` or
...
The :class:`importlib.machinery.WindowsRegistryFinder` kundi is now
...
* ``traceback.Ignore`` kundi and ``traceback.usage``, ``traceback.modname``,
  ``traceback.fullmodname``, ``traceback.find_lines_kutoka_code``,
  ``traceback.find_lines``, ``traceback.find_strings``,
  ``traceback.find_executable_lines`` methods were removed kutoka the
  :mod:`traceback` module. They were undocumented methods deprecated since
  Python 3.2 and equivalent functionality is available kutoka private methods.
...
* The :meth:`~zipfile.ZipFile.open` method of the :class:`zipfile.ZipFile`
  kundi no longer supports the ``'U'`` mode (was deprecated since Python 3.4).
...
* The deprecated ``asynchat.fifo`` kundi has been removed.
...
* The functions in the :mod:`compileall` module now return booleans instead
  ...
  booleans being a subkundi of integers, this should only be an issue if you
...
* The following modules have had missing APIs added to their :attr:`__all__`
  ...
  :mod:`wave`.  This means they will export new symbols when ``agiza *``
  is used.
...
* When a relative agiza is performed and no parent package is known, then
...
* Servers based on the :mod:`socketserver` module, including those
  ...
  kutoka :exc:`Exception`. Therefore if a request handler raises
...
* As part of :pep:`487`, the handling of keyword arguments passed to
  :class:`type` (other than the metakundi hint, ``metaclass``) is now
  ...
  keyword arguments, but :meth:`object.__init_subclass__` (which is called kutoka
...
* In the :mod:`urllib.request` module and the
  ...
  (Contributed by Demian Brecht and Rolf Krahl with tweaks kutoka
...
* The :class:`imaplib.IMAP4` kundi now handles flags containing the
  ``']'`` character in messages sent kutoka the server to improve
...
* when custom metaclasses are combined with zero-argument :func:`super` or
  direct references kutoka methods to the implicit ``__class__`` closure
...
* With the introduction of :exc:`ModuleNotFoundError`, agiza system consumers
  may start expecting agiza system replacements to raise that more specific
  ...
  alternative agiza systems that completely replace :func:`__import__` will
  need to update their implementations to raise the new subkundi when a module
  can't be found at all. Implementors of compliant plugins to the default
  agiza system shouldn't need to make any changes, as the default import
  system will raise the new subkundi when appropriate.
