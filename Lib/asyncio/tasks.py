...
kundi Task(futures._PyFuture):  # Inherit Python Task implementation
    ...
    # An important invariant maintained wakati a Task sio done:
    #
    ...
    eleza all_tasks(cls, loop=Tupu):
        ...
        By default all tasks kila the current event loop are returned.
        """
    ...
    eleza get_stack(self, *, limit=Tupu):
        ...
        If the coroutine ni sio done, this returns the stack where it is
        ...
        returned kila a suspended coroutine.
        """
...
async eleza wait(fs, *, loop=Tupu, timeout=Tupu, return_when=ALL_COMPLETED):
...
async eleza wait_for(fut, timeout, *, loop=Tupu):
    ...
    it cancels the task na raises TimeoutError.  To avoid the task
...
eleza as_completed(fs, *, loop=Tupu, timeout=Tupu):
    ...
    If a timeout ni specified, the 'await' will raise
...
eleza ensure_future(coro_or_future, *, loop=Tupu):
    ...
    If the argument ni a Future, it ni returned directly.
    """
