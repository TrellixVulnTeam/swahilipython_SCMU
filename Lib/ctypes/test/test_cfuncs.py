# A lot of failures kwenye these tests on Mac OS X.
# Byte order related?
...
kundi CFunctions(unittest.TestCase):
    _dll = CDLL(_ctypes_test.__file__)

    eleza S(self):
        rudisha c_longlong.in_dll(self._dll, "last_tf_arg_s").value
    eleza U(self):
        rudisha c_ulonglong.in_dll(self._dll, "last_tf_arg_u").value
    ...
    eleza test_ulonglong_plus(self):
    ...
    eleza test_longdouble_plus(self):
        self._dll.tf_bD.restype = c_longdouble
        self._dll.tf_bD.argtypes = (c_byte, c_longdouble)
        self.assertEqual(self._dll.tf_bD(0, 42.), 14.)
        self.assertEqual(self.S(), 42)

    eleza test_callwithresult(self):
        eleza process_result(result):
            rudisha result * 2
    ...
    eleza test_void(self):
        self._dll.tv_i.restype = Tupu
        self._dll.tv_i.argtypes = (c_int,)
        self.assertEqual(self._dll.tv_i(42), Tupu)
        self.assertEqual(self.S(), 42)
        self.assertEqual(self._dll.tv_i(-42), Tupu)
        self.assertEqual(self.S(), -42)

# The following repeats the above tests ukijumuisha stdcall functions (where
# they are available)
jaribu:
    WinDLL
tatizo NameError:
...
@need_symbol('WinDLL')
kundi stdcallCFunctions(CFunctions):
