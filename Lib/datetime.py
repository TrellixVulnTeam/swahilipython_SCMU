...
# Similarly, a 400-year cycle has an extra leap day over what we'd get kutoka
# pasting together 4 100-year cycles.
assert _DI400Y == 4 * _DI100Y + 1

# OTOH, a 100-year cycle has one fewer leap day than we'd get kutoka
...
def _ord2ymd(n):
    ...
    # kutoka that boundary to n.  Life is much clearer if we subtract 1 kutoka
    ...
    # Now n is the (non-negative) offset, in days, kutoka January 1 of year, to
    ...
    # Now the year is correct, and n is the offset kutoka January 1.  We find
    ...
    # Now the year and month are correct, and n is the offset kutoka the
...
kundi timedelta:
    ...
    def __floordiv__(self, other):
        if sio isinstance(other, (int, timedelta)):
            return NotImplemented
        usec = self._to_microseconds()
        if isinstance(other, timedelta):
    ...
    def __truediv__(self, other):
        if sio isinstance(other, (int, float, timedelta)):
            return NotImplemented
        usec = self._to_microseconds()
        if isinstance(other, timedelta):
            return usec / other._to_microseconds()
...
kundi date:
    ...
    def isocalendar(self):
        ...
        ISO calendar algorithm taken kutoka
...
kundi time:
    ...
    def dst(self):
        """Return 0 if DST is haiko kwenye effect, or the DST offset (as timedelta
...
kundi datetime(date):
    ...
    def kutokatimestamp(cls, t, tz=None):
        ...
        return cls._kutokatimestamp(t, tz ni sio None, tz)
    ...
    def __add__(self, other):
        ...
        if sio isinstance(other, timedelta):
            return NotImplemented
    ...
    def __sub__(self, other):
        ...
        if sio isinstance(other, datetime):
            if isinstance(other, timedelta):
                return self + -other
            return NotImplemented
...
#
# 4. (x+k).s = x.s
#    This follows kutoka #2, and that datimetimetz+timedelta preserves tzinfo.
#
# 5. (x+k).n = x.n + k
...
#
# If [5] ni sio true now, diff = z.d != 0, and z.d is the offset we need to
...
# a dst() offset, and starting *kutoka* a time already in DST (we know z.d != 0),
...
# clock jumps kutoka 1:59 back to 1:00 again, and repeats the 1:MM hour in
