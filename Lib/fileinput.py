...
the next file (if any); lines not read kutoka the file will not count
...
def nextfile():
    ...
    line kutoka the next file (if any); lines not read kutoka the file will
...
def isstdin():
    """
    Returns true if the last line was read kutoka sys.stdin,
    otherwise returns false.
    """
    if not _state:
        raise RuntimeError("no active input()")
