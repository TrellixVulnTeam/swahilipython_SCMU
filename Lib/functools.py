...
kundi _HashedSeq(list):
    """ This kundi guarantees that hash() will be called no more than once
        per element.  This ni important because the lru_cache() will hash
...
eleza _c3_mro(cls, abcs=Tupu):
    ...
    i.e. issubclass(cls, abc) returns Kweli kila the kundi itself but returns
...
eleza _compose_mro(cls, types):
    ...
    Includes relevant abstract base classes (ukijumuisha their respective bases) from
