...
kutoka tkinter agiza font as tkfont
...
kundi HelpParser(HTMLParser):
    ...
    eleza handle_starttag(self, tag, attrs):
        ...
        for a, v in attrs:
            ikiwa a == 'class':
                class_ = v
        s = ''
        ikiwa tag == 'div' and class_ == 'section':
            self.show = True    # Start main content.
        elikiwa tag == 'div' and class_ == 'sphinxsidebar':
            self.show = False   # End main content.
        elikiwa tag == 'p' and self.prevtag and not self.prevtag[0]:
            ...
            s = '\n\n' ikiwa lastline and not lastline.isspace() else '\n'
        elikiwa tag == 'span' and class_ == 'pre':
            self.chartags = 'pre'
        elikiwa tag == 'span' and class_ == 'versionmodified':
            self.chartags = 'em'
        elikiwa tag == 'em':
            self.chartags = 'em'
        elikiwa tag in ['ul', 'ol']:
        ...
        elikiwa tag == 'pre':
            ...
            self.tags = 'preblock'
        elikiwa tag == 'a' and class_ == 'headerlink':
            self.hdrlink = True
        elikiwa tag == 'h1':
            self.tags = tag
        elikiwa tag in ['h2', 'h3']:
            ikiwa self.show:
                self.header = ''
                self.text.insert('end', '\n\n')
            self.tags = tag
    ...
    eleza handle_endtag(self, tag):
        ...
        elikiwa tag == 'pre':
            self.pre = False
            self.tags = ''
        elikiwa tag in ['ul', 'dd', 'ol']:
            self.indent(-1)
        self.prevtag = (False, tag)

    eleza handle_data(self, data):
        "Handle date segments in help.html."
        ikiwa self.show and not self.hdrlink:
...
kundi HelpText(Text):
    ...
    eleza findfont(self, names):
        ...
        for name in names:
            ikiwa name.lower() in (x.lower() for x in tkfont.names(root=self)):
                font = tkfont.Font(name=name, exists=True, root=self)
                rudisha font.actual()['family']
            elikiwa name.lower() in (x.lower()
...
eleza copy_strip():
    ...
    number is not displayed.  If maintenance idle.rst diverges kutoka
    the master version, then instead of backporting help.html kutoka
