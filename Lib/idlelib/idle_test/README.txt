...
Remove the agizas of requires and tkinter ikiwa not needed.  Otherwise,
add to the tkinter agizas as needed.
...
section ikiwa doing gui tests.  If not, and not needed for further classes,
this code can be removed.

Add the following at the end of abc.py.  If an htest was added first,
insert the agiza and main lines before the htest lines.

ikiwa __name__ == "__main__":
    kutoka unittest agiza main
    main('idlelib.idle_test.test_abc', verbosity=2, exit=False)

The ', exit=False' is only needed ikiwa an htest follows.
...
test.support.requires('gui').  A test is a GUI test ikiwa it creates a
...
the module or kundi attribute deleted.  Others widgets should descend
kutoka the single root and the attributes deleted BEFORE root is
...
(#27196).  It should not hurt ikiwa not needed.
...
Running either kutoka an Idle editor runs all tests in the test_xyz file
...
The test suite can be run in an IDLE user process kutoka Shell.
...
htest file kutoka an editor or kutoka the command line with:
...
coverage uses absolute agizas for its submodules, hence it needs to be
...
coverage either directly or kutoka a script in that directory so that
...
[report]
# Regexes for lines to exclude kutoka consideration
exclude_lines =
...
rem filename without .py, 2nd parameter ikiwa test is not test_filename
setlocal
