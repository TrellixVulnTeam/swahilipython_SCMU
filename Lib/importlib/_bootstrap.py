...
def _verbose_message(message, *args, verbosity=1):
    ...
    if sys.flags.verbose >= verbosity:
        if not message.startswith(('#', 'agiza ')):
            message = '# ' + message
...
def _module_repr(module):
    ...
    try:
        ...
        return '<module {!r} kutoka {!r}>'.format(name, filename)
...
def _module_repr_from_spec(spec):
    ...
    if spec.origin is None:
        ...
        if spec.has_location:
            return '<module {!r} kutoka {!r}>'.format(name, spec.origin)
        else:
...
def _gcd_agiza(name, package=None, level=0):
    ...
    return _find_and_load(name, _gcd_agiza)
    ...
    for x in fromlist:
        if not isinstance(x, str):
            ...
            try:
                ...
                # agizas triggered by fromlist for modules that don't
                # exist.
...
def __import__(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    if not fromlist:
        ...
        if level == 0:
            return _gcd_agiza(name.partition('.')[0])
        elif not name:
            return module
        else:
        ...
        return _handle_fromlist(module, fromlist, _gcd_agiza)
    else:
        return module
