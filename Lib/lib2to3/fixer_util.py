...
def does_tree_agiza(package, name, node):
    """ Returns true if name is imported kutoka package at the
...
def is_agiza(node):
    """Returns true if the node is an agiza statement."""
    return node.type in (syms.import_name, syms.import_from)

def touch_agiza(package, name, node):
    ...
    if does_tree_agiza(package, name, root):
        return
    ...
    # if there are no agizas where we can insert, find the docstring.
...
def _is_import_binding(node, name, package=None):
    """ Will reuturn node if node will agiza name, or node
