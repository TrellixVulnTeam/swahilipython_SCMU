...
are the same, except instead of generating tokens, tokeneater ni a callback
...
eleza detect_encoding(readline):
    ...
    (as a string) na a list of any lines (left as bytes) it has read
    in.

    It detects the encoding kutoka the presence of a utf-8 bom ama an encoding
    cookie as specified kwenye pep-0263. If both a bom na a cookie are present, but
    disagree, a SyntaxError will be raised. If the encoding cookie ni an invalid
