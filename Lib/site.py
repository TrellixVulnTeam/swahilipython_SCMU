...
def removeduppaths():
    ...
    # only absolute pathnames, even if we're running kutoka the build directory.
    L = []
    known_paths = set()
    for dir in sys.path:
...
def addpackage(sitedir, name, known_paths):
    """Process a .pth file within the site-packages directory:
       ...
       and add that to known_paths, or execute it if it starts with 'agiza '.
    """
    ...
    with f:
        for n, line in enumerate(f):
            if line.startswith("#"):
                continue
            try:
                if line.startswith(("agiza ", "agiza\t")):
                    exec(line)
                    continue
                line = line.rstrip()
