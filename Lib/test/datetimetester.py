...
kundi TestModule(unittest.TestCase):
    ...
    eleza test_name_cleanup(self):
        ikiwa '_Pure' in self.__class__.__name__:
            self.skipTest('Only run for Fast C implementation')

        datetime = datetime_module
        names = set(name for name in dir(datetime)
                    ikiwa not name.startswith('__') and not name.endswith('__'))
    ...
    eleza test_divide_and_round(self):
        ikiwa '_Fast' in self.__class__.__name__:
...
kundi PicklableFixedOffset(FixedOffset):
    ...
    eleza __init__(self, offset=None, name=None, dstoffset=None):
...
#############################################################################
# Base kundi for testing a particular aspect of timedelta, time, date and
...
kundi TestDate(HarmlessMixedComparison, unittest.TestCase):
    ...
    eleza test_today(self):
        ...
        # We claim that today() is like kutokatimestamp(time.time()), so
        # prove it.
    ...
    eleza test_iso_long_years(self):
        # Calculate long ISO years and compare to table kutoka
        # http://www.phys.uu.nl/~vgent/calendar/isocalendar.htm
    ...
    eleza test_mixed_compare(self):
        ...
        # Our kundi can be compared for equality to other classes
    ...
    eleza test_backdoor_resistance(self):
        ...
        base = b'1995-03-25'
        ikiwa not issubclass(self.theclass, datetime):
            base = base[:4]
    ...
    eleza test_kutokaisoformat_fails(self):
        ...
        bad_strs = [
            ...
            '20090228',         # Valid ISO8601 output not kutoka isoformat()
    ...
    eleza test_kutokaisoformat_fails_typeerror(self):
        # Test that kutokaisoformat fails when passed the wrong type
        agiza io
    ...
    eleza test_kutokaisocalendar(self):
        # For each test case, assert that kutokaisocalendar is the
...
kundi TestDateTime(TestDate):
    ...
    eleza test_microsecond_rounding(self):
        for fts in [self.theclass.kutokatimestamp,
    ...
    eleza test_utcnow(self):
        ...
        # Call it a success ikiwa utcnow() and utckutokatimestamp() are within
    ...
    eleza test_strptime(self):
        ...
        # Produce naive datetime ikiwa no %z is provided
    ...
    eleza test_astimezone(self):
        ...
        kundi Bogus(tzinfo):
            eleza utcoffset(self, dt): rudisha None
        ...
        kundi AlsoBogus(tzinfo):
            ...
            eleza dst(self, dt): rudisha None
        alsobog = AlsoBogus()
...
# thekundi as a kundi attribute, and theclass(1, 1, 1, tzinfo=whatever)
...
kundi TZInfoBase:
    ...
    eleza test_argument_passing(self):
        ...
        kundi introspective(tzinfo):
            eleza tzname(self, dt):    rudisha dt and "real" or "none"
            eleza utcoffset(self, dt):
                rudisha timedelta(minutes = dt and 42 or -42)
            dst = utcoffset
    ...
    eleza test_tzinfo_classes(self):
        ...
        kundi C1(tzinfo):
            eleza utcoffset(self, dt): rudisha None
            eleza dst(self, dt): rudisha None
            eleza tzname(self, dt): rudisha None
        for t in (cls(1, 1, 1),
    ...
    eleza test_aware_compare(self):
        ...
        # However, ikiwa they're different members, uctoffset is not ignored.
        # Note that a time can't actually have an operand-dependent offset,
        # though (and time.utcoffset() passes None to tzinfo.utcoffset()),
        # so skip this test for time.
        ikiwa cls is not time:
            ...
            for x in d0, d1, d2:
                for y in d0, d1, d2:
                    got = (x > y) - (x < y)
                    ikiwa (x is d0 or x is d1) and (y is d0 or y is d1):
                        expected = 0
                    elikiwa x is y is d2:
                        expected = 0
                    elikiwa x is d2:
                        expected = -1
                    else:
...
kundi TestTimeTZ(TestTime, TZInfoBase, unittest.TestCase):
    ...
    eleza test_mixed_compare(self):
        ...
        # But ikiwa they're not identical, it isn't ignored.
    ...
    eleza test_kutokaisoformat_fails_typeerror(self):
        # Test the kutokaisoformat fails when passed the wrong type
        agiza io
...
kundi TestDateTimeTZ(TestDateTime, TZInfoBase, unittest.TestCase):
    ...
    eleza test_even_more_compare(self):
        # The test_compare() and test_more_compare() inherited kutoka TestDate
        ...
        # It's also naive ikiwa it has tzinfo but tzinfo.utcoffset() is None.
        kundi Naive(tzinfo):
            eleza utcoffset(self, dt): rudisha None
    ...
    eleza test_utctimetuple(self):
        ...
        kundi UOFS(DST):
            eleza __init__(self, uofs, dofs=None):
    ...
    eleza test_more_astimezone(self):
        ...
        expected += fm5h.utcoffset(dt)  # and kutoka there to local time
...
kundi USTimeZone(tzinfo):
    ...
    eleza dst(self, dt):
        ikiwa dt is None or dt.tzinfo is None:
        ...
        # dt first.
        ikiwa start <= dt.replace(tzinfo=None) < end:
            rudisha HOUR
        else:
            rudisha ZERO
...
kundi TestTimezoneConversions(unittest.TestCase):
    ...
    eleza checkinside(self, dt, tz, utc, dston, dstoff):
        ...
        ikiwa dt.date() == dston.date() and dt.hour == 2:
            # We're in the redundant hour, and coming back kutoka
        ...
        ikiwa dt.date() == dstoff.date() and dt.hour == 0:
    ...
    eleza test_easy(self):
        ...
        # kundi (lke utc_real and utc_fake), but not for Eastern or Central.
    ...
    eleza test_tricky(self):
        ...
        # local clock jumps kutoka 1 to 3).  The point here is to make sure we
        # get the 3 spelling.
    ...
    eleza test_bogus_dst(self):
        ...
        # implementation may rudisha None or not None depending on
    ...
    eleza test_kutokautc(self):
        self.assertRaises(TypeError, Eastern.kutokautc)   # not enough args
...
kundi Oddballs(unittest.TestCase):
    ...
    eleza test_bug_1028306(self):
        ...
        # type comparison, despite that datetime is a subkundi of date.
        as_date = date.today()
        ...
        # projection ikiwa use of a date method is forced.
...
kundi ZoneInfo(tzinfo):
    ...
    eleza dst(self, dt):
        ...
        # so let's rudisha 1h whenever DST is in effect.  Since
        # we don't use dst() in kutokautc(), it is unlikely that
    ...
    @classmethod
    eleza zonenames(cls, zonedir=None):
        ikiwa zonedir is None:
        ...
        with f:
            for line in f:
                line = line.strip()
                ikiwa line and not line.startswith('#'):
                    yield line.split()[2]
    ...
    eleza stats(cls, start_year=1):
        ...
        andika("Min gap:         %16s at %s in %s" % (min_gap, min_gap_datetime, min_gap_zone))
        andika("Max gap:         %16s at %s in %s" % (max_gap, max_gap_datetime, max_gap_zone))
        andika("Min fold:        %16s at %s in %s" % (min_fold, min_fold_datetime, min_fold_zone))
        andika("Max fold:        %16s at %s in %s" % (max_fold, max_fold_datetime, max_fold_zone))


    eleza transitions(self):
        for (_, prev_ti), (t, ti) in pairs(zip(self.ut, self.ti)):
            shift = ti[0] - prev_ti[0]
            yield datetime.utckutokatimestamp(t), shift

    eleza nondst_folds(self):
        """Find all folds with the same value of isdst on both sides of the transition."""
        for (_, prev_ti), (t, ti) in pairs(zip(self.ut, self.ti)):
            shift = ti[0] - prev_ti[0]
            ikiwa shift < ZERO and ti[1] == prev_ti[1]:
                yield datetime.utckutokatimestamp(t), -shift, prev_ti[2], ti[2]

    @classmethod
    eleza print_all_nondst_folds(cls, same_abbr=False, start_year=1):
        count = 0
        for zonename in cls.zonenames():
            tz = cls.kutokaname(zonename)
            for dt, shift, prev_abbr, abbr in tz.nondst_folds():
                ikiwa dt.year < start_year or same_abbr and prev_abbr != abbr:
                    continue
                count += 1
...
kundi ZoneInfoTest(unittest.TestCase):
    ...
    eleza test_system_transitions(self):
        ikiwa ('Riyadh8' in self.zonename or
        ...
        try:
            ...
            for udt, shift in tz.transitions():
                ...
                for x in [-40 * 3600, -20*3600, -1, 0,
                    ...
                    tzdt = datetime.kutokatimestamp(s, tz).replace(tzinfo=None)
...
kundi CapiTest(unittest.TestCase):
    ...
    eleza test_check_time(self):
        ...
        # Check that the subkundi does not match exactly
