...
kundi BaseTestCase(unittest.TestCase):
    ...
    eleza request(self, uri, method='GET', body=None, headers={}):
...
kundi BaseHTTPServerTestCase(BaseTestCase):
    ...
    eleza test_send_error(self):
        ...
        for code in (HTTPStatus.NO_CONTENT, HTTPStatus.NOT_MODIFIED,
            ...
            ikiwa code not in allow_transfer_encoding_codes:
...
kundi SimpleHTTPServerTestCase(BaseTestCase):
    ...
    eleza check_status_and_reason(self, response, status, data=None):
        eleza close_conn():
    ...
    eleza test_undecodable_filename(self):
        ...
        ikiwa sys.platform == 'darwin':
            ...
            for name in os.listdir(self.tempdir):
                ikiwa name != 'test': # Ignore a filename created in setUp().
                    filename = name
                    break
    ...
    eleza test_get(self):
        ...
        ikiwa os.name == 'posix' and os.geteuid() != 0:
    ...
    eleza test_browser_cache_with_If_None_Match_header(self):
        # ikiwa If-None-Match header is present, ignore If-Modified-Since
...
kundi CGIHTTPServerTestCase(BaseTestCase):
    ...
    eleza test_url_collapse_path(self):
        ...
        for path, expected in test_vectors.items():
            ikiwa isinstance(expected, type) and issubclass(expected, Exception):
...
kundi BaseHTTPRequestHandlerTestCase(unittest.TestCase):
    ...
    eleza test_with_continue_1_0(self):
        result = self.send_typical_request(b'GET / HTTP/1.0\r\nExpect: 100-continue\r\n\r\n')
        self.verify_http_server_response(result[0])
        self.verify_expected_headers(result[1:-1])
        self.verify_get_called()
    ...
    eleza test_with_continue_1_1(self):
        result = self.send_typical_request(b'GET / HTTP/1.1\r\nExpect: 100-continue\r\n\r\n')
    ...
    eleza test_header_unbuffered_when_continue(self):
    ...
    eleza test_with_continue_rejected(self):
...
kundi MiscTestCase(unittest.TestCase):
    eleza test_all(self):
        ...
        for name in dir(server):
            ...
            ikiwa getattr(module_object, '__module__', None) == 'http.server':
