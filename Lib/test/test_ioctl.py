...
try:
    ...
    with tty:
        # Skip ikiwa another process is in foreground
        r = fcntl.ioctl(tty, termios.TIOCGPGRP, "    ")
    rpgrp = struct.unpack("i", r)[0]
    ikiwa rpgrp not in (os.getpgrp(), os.getsid(0)):
...
kundi IoctlTests(unittest.TestCase):
    ...
    eleza _check_ioctl_mutate_len(self, nbytes=None):
        buf = array.array('i')
        intsize = buf.itemsize
        ids = (os.getpgrp(), os.getsid(0))
        # A fill value unlikely to be in `ids`
        fill = -12345
        ikiwa nbytes is not None:
