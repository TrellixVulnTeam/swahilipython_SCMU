...
kundi TestDecode:
    ...
    eleza test_object_pairs_hook(self):
    ...
    eleza check_keys_reuse(self, source, loads):
        rval = loads(source)
        (a, b), (c, d) = sorted(rval[0]), sorted(rval[1])
        self.assertIs(a, c)
        self.assertIs(b, d)

    eleza test_keys_reuse(self):
    ...
    eleza test_invalid_escape(self):
        s = '["abc\\y"]'
        msg = 'escape'
        self.assertRaisesRegex(self.JSONDecodeError, msg, self.loads, s)
    ...
    eleza test_negative_index(self):
