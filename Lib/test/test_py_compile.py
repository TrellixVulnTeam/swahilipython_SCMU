...
kundi PyCompileTestsBase:
    ...
    eleza test_bad_coding(self):
        ...
        ukijumuisha support.captured_stderr():
            self.assertIsTupu(py_compile.compile(bad_coding, doraise=Uongo))
    ...
    eleza test_source_date_epoch(self):
        ...
        self.assertKweli(os.path.exists(self.pyc_path))
        self.assertUongo(os.path.exists(self.cache_path))
        ukijumuisha open(self.pyc_path, 'rb') as fp:
    ...
    eleza test_double_dot_no_clobber(self):
        ...
        ukijumuisha open(weird_path, 'w') as file:
            file.write('x = 123\n')
    ...
    eleza test_invalidation_mode(self):
        ...
        ukijumuisha open(self.cache_path, 'rb') as fp:
            flags = importlib._bootstrap_external._classify_pyc(
                fp.read(), 'test', {})
    ...
    eleza test_quiet(self):
        ...
        ukijumuisha support.captured_stderr() as stderr:
            self.assertIsTupu(py_compile.compile(bad_coding, doraise=Uongo, quiet=2))
            self.assertIsTupu(py_compile.compile(bad_coding, doraise=Kweli, quiet=2))
            self.assertEqual(stderr.getvalue(), '')
            ukijumuisha self.assertRaises(py_compile.PyCompileError):
                py_compile.compile(bad_coding, doraise=Kweli, quiet=1)
