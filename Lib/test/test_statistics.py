...
eleza _nan_equal(a, b):
    """Return True ikiwa a and b are both the same kind of NAN.
    ...
    """
    ikiwa type(a) is not type(b):
        rudisha False
    ikiwa isinstance(a, float):
        rudisha math.isnan(a) and math.isnan(b)
    aexp = a.as_tuple()[2]
    bexp = b.as_tuple()[2]
    rudisha (aexp == bexp) and (aexp in ('n', 'N'))  # Both NAN or both sNAN.
...
eleza _calc_errors(actual, expected):
    ...
    rel_err = abs_err/base ikiwa base else float('inf')
...
eleza approx_equal(x, y, tol=1e-12, rel=1e-7):
    ...
    Return True ikiwa numbers x and y are approximately equal, to within some
    margin of error, otherwise rudisha False. Numbers which compare equal
    will also compare approximately equal.

    x is approximately equal to y ikiwa the difference between them is less than
    ...
    Absolute error is defined as abs(x-y); ikiwa that is less than or equal to
    ...
    ikiwa x == y:
        ...
        rudisha True
    ikiwa math.isinf(x) or math.isinf(y):
...
# docstring. Ultimately, this kundi will die, and the information below will
...
kundi NumericTestCase(unittest.TestCase):
    """Unit test kundi for numeric work.
    ...
    eleza assertApproxEqual(
        ...
        """Test passes ikiwa ``first`` and ``second`` are approximately equal.

        This test passes ikiwa ``first`` and ``second`` are equal to
    ...
    eleza _check_approx_num(self, first, second, tol, rel, msg, idx=None):
...
kundi ApproxEqualInexactTest(unittest.TestCase):
    ...
    eleza do_check_both(self, a, b, tol, rel, tol_flag, rel_flag):
        check = self.assertTrue ikiwa tol_flag else self.assertFalse
        check(approx_equal(a, b, tol=tol, rel=0))
        check = self.assertTrue ikiwa rel_flag else self.assertFalse
        check(approx_equal(a, b, tol=0, rel=rel))
        check = self.assertTrue ikiwa (tol_flag or rel_flag) else self.assertFalse
...
kundi CoerceTest(unittest.TestCase):
    ...
    #
    #   - coerce(T, T) will always rudisha T; we assume T is a valid numeric
    #     type. Violate this assumption at your own risk.
    #
    #   - Apart kutoka as above, bool is treated as ikiwa it were actually int.
    #
    #   - coerce(int, X) and coerce(X, int) rudisha X.
    #   -
    ...
    eleza check_type_coercions(self, T):
        ...
        # Coercing a type with a subkundi of itself returns the subclass.
        kundi U(T): pass
        kundi V(T): pass
        kundi W(U): pass
        for typ in (U, V, W):
...
kundi UnivariateTypeMixin:
    """Mixin kundi for type-conserving functions.

    This mixin kundi holds test(s) for functions which conserve the type of
...
kundi AverageMixin(UnivariateCommonMixin):
    # Mixin kundi holding common tests for averages.
...
kundi TestMode(NumericTestCase, AverageMixin, UnivariateTypeMixin):
    ...
    eleza test_none_data(self):
        # Test that mode raises TypeError ikiwa given None as data.
...
kundi TestFMean(unittest.TestCase):
    ...
    eleza test_special_values(self):
        # Rules for special values are inherited kutoka math.fsum()
        fmean = statistics.fmean
        NaN = float('Nan')
        Inf = float('Inf')
...
kundi VarianceStdevMixin(UnivariateCommonMixin):
    # Mixin kundi holding common tests for variance and std dev.

    # Subclasses should inherit kutoka this before NumericTestClass, in order
...
kundi TestQuantiles(unittest.TestCase):
    ...
    eleza test_specific_cases(self):
        ...
        for n, expected in [
            ...
            # extrapolated kutoka the two lowest and two highest points.
            sdata = sorted(data)
...
kundi TestNormalDist:
    ...
    eleza test_instantiation_and_attributes(self):
        ...
        # verify that subkundi type is honored
    ...
    eleza test_overlap(self):
        ...
        # Match examples kutoka Imman and Bradley
        ...
        for X1, X2 in [
                # Examples kutoka Imman and Bradley
