...
# Some constructs like "while 0:", "ikiwa 0:" or "ikiwa 1:...else:..." are optimized
# away.  No code # exists for them, so the line numbers skip directly kutoka
# "del x" to "x = 1".
...
eleza _settrace_and_return(tracefunc):
    sys.settrace(tracefunc)
    sys._getframe().f_back.f_trace = tracefunc
eleza settrace_and_return(tracefunc):
...
# The compiler then generates an implicit "rudisha None"
# Internally, the compiler visits the pass statement
# and stores its line number for use on the next instruction.
# The next instruction is the implicit rudisha None.
...
kundi TraceTestCase(unittest.TestCase):
    ...
    eleza test_06_call(self):
        self.run_test(call)
    eleza test_07_raise(self):
        self.run_test(test_raise)

    eleza test_08_settrace_and_return(self):
        self.run_test2(settrace_and_return)
    eleza test_09_settrace_and_raise(self):
        self.run_test2(settrace_and_raise)
    eleza test_10_ireturn(self):
    ...
    eleza test_13_genexp(self):
        ...
        # and ikiwa the traced function contains another generator
...
kundi SkipLineEventsTraceTestCase(TraceTestCase):
    ...
    eleza compare_events(self, line_offset, events, expected_events):
        skip_line_events = [e for e in expected_events ikiwa e[1] != 'line']
        super().compare_events(line_offset, events, skip_line_events)

    @staticmethod
    eleza make_tracer():
        rudisha Tracer(trace_line_events=False)
...
kundi TraceOpcodesTestCase(TraceTestCase):
    ...
    eleza compare_events(self, line_offset, events, expected_events):
        skip_opcode_events = [e for e in events ikiwa e[1] != 'opcode']
        ikiwa len(events) > 1:
    ...
    @staticmethod
    eleza make_tracer():
        rudisha Tracer(trace_opcode_events=True)
...
kundi RaisingTraceFuncTestCase(unittest.TestCase):
    ...
    eleza f(self):
        """The function to trace; raises an exception ikiwa that's the case
        we're testing, so that the 'exception' trace event fires."""
        ikiwa self.raiseOnEvent == 'exception':
            x = 0
            y = 1/x
        else:
            rudisha 1
    ...
    eleza test_trash_stack(self):
        eleza f():
            for i in range(5):
                andika(i)  # line tracing will raise an exception at this line
...
kundi JumpTracer:
    ...
    eleza __init__(self, function, jumpFrom, jumpTo, event='line',
        ...
        self.jumpTo = jumpTo
        self.event = event
        self.firstLine = None ikiwa decorated else self.code.co_firstlineno
        self.done = False
    ...
    eleza trace(self, frame, event, arg):
        ...
        ikiwa (self.firstLine is None and frame.f_code == self.code and
                event == 'line'):
            self.firstLine = frame.f_lineno - 1
        ikiwa (event == self.event and self.firstLine and
...
kundi JumpTestCase(unittest.TestCase):
    ...
    eleza run_test(self, func, jumpFrom, jumpTo, expected, error=None,
    ...
    eleza run_async_test(self, func, jumpFrom, jumpTo, expected, error=None,
                 event='line', decorated=False):
        tracer = JumpTracer(func, jumpFrom, jumpTo, event, decorated)
        sys.settrace(tracer.trace)
        output = []
    ...
    eleza jump_test(jumpFrom, jumpTo, expected, error=None, event='line'):
        """Decorator that creates a test that makes a jump
        kutoka one place to another in the following code.
        """
    ...
    eleza async_jump_test(jumpFrom, jumpTo, expected, error=None, event='line'):
        """Decorator that creates a test that makes a jump
        kutoka one place to another in the following asynchronous code.
        """
        eleza decorator(func):
            @wraps(func)
            eleza test(self):
    ...
    eleza test_jump_over_return_try_finally_in_finally_block(output):
        try:
            output.append(2)
        finally:
            output.append(4)
    ...
    eleza test_jump_over_return_in_try_finally_block(output):
        output.append(1)
        try:
            output.append(3)
            ikiwa not output: # always false
                return
            output.append(6)
        finally:
            output.append(8)
    ...
    eleza test_jump_over_break_in_try_finally_block(output):
        output.append(1)
        while True:
            output.append(3)
            try:
    ...
    eleza test_no_jump_into_finally_block_kutoka_try_block(output):
        try:
            output.append(2)
            output.append(3)
        finally:  # still executed ikiwa the jump is failed
            output.append(5)
            output.append(6)
        output.append(7)
    ...
    eleza test_no_jump_into_bare_except_block_kutoka_try_block(output):
        try:
            output.append(2)
            output.append(3)
        except:  # executed ikiwa the jump is failed
            output.append(5)
            output.append(6)
            raise
        output.append(8)
    ...
    eleza test_no_jump_over_return_out_of_finally_block(output):
        try:
            output.append(2)
        finally:
            output.append(4)
    ...
    eleza test_jump_to_firstlineno(self):
        ...
        # function call ikiwa the function is defined on a single line.
        code = compile("""
...
""", "<fake module>", "exec")
    ...
    eleza test_no_jump_kutoka_return_event(output):
        output.append(1)
        return
    ...
    @jump_test(3, 2, [2], event='return', error=(ValueError,
               "can't jump kutoka a yield statement"))
    eleza test_no_jump_kutoka_yield(output):
        eleza gen():
            output.append(2)
            yield 3
        next(gen())
