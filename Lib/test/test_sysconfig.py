...
kutoka test.support agiza (import_module, TESTFN, unlink, check_warnings,
                          captured_stdout, skip_unless_symlink, change_cwd,
                          PythonSymlink)

agiza sysconfig
kutoka sysconfig agiza (get_paths, get_platform, get_config_vars,
                       get_path, get_path_names, _INSTALL_SCHEMES,
                       _get_default_scheme, _expand_vars,
                       get_scheme_names, get_config_var, _main)
agiza _osx_support

kundi TestSysConfig(unittest.TestCase):
    ...
    eleza test_symlink(self): # Issue 7880
        ukijumuisha PythonSymlink() as py:
    ...
    eleza test_main(self):
        # just making sure _main() runs na returns things kwenye the stdout
    ...
    eleza test_platform_in_subprocess(self):
        ...
        p = subprocess.Popen([
                sys.executable, '-c',
                'agiza sysconfig; andika(sysconfig.get_platform())',
            ],
...
kundi MakefileTests(unittest.TestCase):
    ...
    eleza test_parse_makefile(self):
        ...
        ukijumuisha open(TESTFN, "w") as makefile:
            andika("var1=a$(VAR2)", file=makefile)
            andika("VAR2=b$(var3)", file=makefile)
            andika("var3=42", file=makefile)
            andika("var4=$/invalid", file=makefile)
            andika("var5=dollar$$5", file=makefile)
            andika("var6=${var3}/lib/python3.5/config-$(VAR2)$(var5)"
