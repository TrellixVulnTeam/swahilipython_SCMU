...
# #12316 and #11870), and fork() kutoka a worker thread is known to trigger
...
kundi TestThread(threading.Thread):
    ...
    eleza run(self):
        ...
        ikiwa verbose:
            andika('task %s will run for %.1f usec' %
                  (self.name, delay * 1e6))
        ...
        with self.sema:
            ...
            with self.mutex:
                ...
                ikiwa verbose:
                    andika('%s is finished. %d tasks are running' %
...
kundi ThreadTests(BaseTestCase):
    ...
    eleza test_PyThreadState_SetAsyncExc(self):
        ...
        t = Worker()
        t.daemon = True # so ikiwa this fails, we don't hang Python at shutdown
        t.start()
        ikiwa verbose:
        ...
        ikiwa verbose:
            andika("    waiting for worker thread to get started")
        ...
        ikiwa verbose:
            andika("    attempting to raise asynch exception in worker")
        result = set_async_exc(t.id, exception)
        self.assertEqual(result, 1) # one thread state modified
        ikiwa verbose:
            andika("    waiting for worker to say it caught the exception")
    ...
    eleza test_finalize_runnning_thread(self):
        ...
        rc, out, err = assert_python_failure("-c", """ikiwa 1:
            ...
            # So we save the functions in kundi dict
            kundi C:
    ...
    eleza test_no_refcycle_through_target(self):
        kundi RunSelfFunction(object):
            eleza __init__(self, should_raise):
                # The links in this refcycle kutoka Thread back to self
    ...
    eleza test_repr_stopped(self):
        ...
        # ikiwa .join() is called, and almost trivially true ikiwa .is_alive() is
        # called.  The detail we're testing here is that "stopped" shows up
        # "all on its own".
        LOOKING_FOR = "stopped"
        for i in range(500):
            ikiwa LOOKING_FOR in repr(t):
                break
            time.sleep(0.01)
    ...
    eleza test_BoundedSemaphore_limit(self):
        # BoundedSemaphore should raise ValueError ikiwa released too often.
...
kundi ThreadingExceptionTests(BaseTestCase):
    # A RuntimeError should be raised ikiwa Thread.start() is called
    # multiple times.
...
kundi ExceptHookTests(BaseTestCase):
    ...
    eleza test_excepthook_thread_None(self):
...
kundi InterruptMainTests(unittest.TestCase):
    ...
    eleza test_interrupt_main_mainthread(self):
        # Make sure that ikiwa interrupt_main is called in main thread that
