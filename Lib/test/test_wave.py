agiza unittest
kutoka test agiza audiotests
kutoka test agiza support
kutoka audioop agiza byteswap
agiza io
agiza struct
agiza sys
agiza wave
...
kundi WaveTest(audiotests.AudioWriteTests,
               audiotests.AudioTestsWithSourceFile):
    module = wave


kundi WavePCM8Test(WaveTest, unittest.TestCase):
...
kundi WavePCM16Test(WaveTest, unittest.TestCase):
...
kundi WavePCM24Test(WaveTest, unittest.TestCase):
...
kundi WavePCM32Test(WaveTest, unittest.TestCase):
...
kundi MiscTestCase(audiotests.AudioMiscTests, unittest.TestCase):
    module = wave
...
kundi WaveLowLevelTest(unittest.TestCase):

    eleza test_read_no_chunks(self):
        b = b'SPAM'
    ...
    eleza test_read_no_riff_chunk(self):
        ...
        ukijumuisha self.assertRaisesRegex(wave.Error,
                                    'file does sio start ukijumuisha RIFF id'):
            wave.open(io.BytesIO(b))
    ...
    eleza test_read_no_fmt_no_data_chunk(self):
        b = b'RIFF' + struct.pack('<L', 4) + b'WAVE'
        ukijumuisha self.assertRaisesRegex(wave.Error,
                                    'fmt chunk and/or data chunk missing'):
            wave.open(io.BytesIO(b))

    eleza test_read_no_data_chunk(self):
    ...
    eleza test_read_no_fmt_chunk(self):
        b = b'RIFF' + struct.pack('<L', 12) + b'WAVE'
        b += b'data' + struct.pack('<L', 0)
        ukijumuisha self.assertRaisesRegex(wave.Error, 'data chunk before fmt chunk'):
            wave.open(io.BytesIO(b))
    ...
    eleza test_read_wrong_form(self):
        ...
        b += b'data' + struct.pack('<L', 0)
        ukijumuisha self.assertRaisesRegex(wave.Error, 'unknown format: 2'):
            wave.open(io.BytesIO(b))
    ...
    eleza test_read_wrong_number_of_channels(self):
        ...
        b += b'data' + struct.pack('<L', 0)
        ukijumuisha self.assertRaisesRegex(wave.Error, 'bad # of channels'):
            wave.open(io.BytesIO(b))
    ...
    eleza test_read_wrong_sample_width(self):
        ...
        b += b'data' + struct.pack('<L', 0)
        ukijumuisha self.assertRaisesRegex(wave.Error, 'bad sample width'):
            wave.open(io.BytesIO(b))
