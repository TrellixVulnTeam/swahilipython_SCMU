...
kundi TestWeakSet(unittest.TestCase):
    ...
    eleza setUp(self):
        ...
        self.d = dict.kutokakeys(self.items)
        self.obj = ustr('F')
    ...
    eleza test_methods(self):
        ...
        for method in dir(set):
            ikiwa method == 'test_c_api' or method.startswith('_'):
                continue
    ...
    eleza test_union(self):
        ...
        for C in set, frozenset, dict.kutokakeys, list, tuple:
    ...
    eleza test_intersection_update(self):
        ...
        self.assertEqual(retval, None)
        for c in (self.items + self.items2):
            ikiwa c in self.items2 and c in self.items:
                self.assertIn(c, self.s)
            else:
    ...
    eleza test_difference_update(self):
        ...
        self.assertEqual(retval, None)
        for c in (self.items + self.items2):
            ikiwa c in self.items and c not in self.items2:
                self.assertIn(c, self.s)
            else:
                self.assertNotIn(c, self.s)
    ...
    eleza test_symmetric_difference_update(self):
        ...
        self.assertEqual(retval, None)
        for c in (self.items + self.items2):
            ikiwa (c in self.items) ^ (c in self.items2):
                self.assertIn(c, self.s)
            else:
                self.assertNotIn(c, self.s)
        self.assertRaises(TypeError, self.s.symmetric_difference_update, [[]])
