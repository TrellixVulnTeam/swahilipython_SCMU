...
kundi AbstractTestsWithSourceFile:
    ...
    eleza make_test_archive(self, f, compression, compresslevel=Tupu):
        ...
        ukijumuisha zipfile.ZipFile(f, "w", **kwargs) as zipfp:
            zipfp.write(TESTFN, "another.name")
            zipfp.write(TESTFN, TESTFN)
            zipfp.writestr("strfile", self.data)
            ukijumuisha zipfp.open('written-open-w', mode='w') as f:
    ...
    eleza zip_open_test(self, f, compression):
        ...
        ukijumuisha zipfile.ZipFile(f, "r", compression) as zipfp:
            zipdata1 = []
            ukijumuisha zipfp.open(TESTFN) as zipopen1:
                wakati Kweli:
            ...
            zipdata2 = []
            ukijumuisha zipfp.open("another.name") as zipopen2:
                wakati Kweli:
    ...
    eleza test_open_with_pathlike(self):
        ...
        ukijumuisha zipfile.ZipFile(path, "r", self.compression) as zipfp:
    ...
    eleza zip_read1_test(self, f, compression):
        ...
        # Read the ZIP archive
        ukijumuisha zipfile.ZipFile(f, "r") as zipfp, \
    ...
    eleza zip_readline_test(self, f, compression):
        ...
        # Read the ZIP archive
        ukijumuisha zipfile.ZipFile(f, "r") as zipfp:
            ukijumuisha zipfp.open(TESTFN) as zipopen:
                kila line kwenye self.line_gen:
    ...
    eleza test_low_compression(self):
        ...
        # Create the ZIP archive
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", self.compression) as zipfp:
            zipfp.writestr("strfile", '12')

        # Get an open object kila strfile
        ukijumuisha zipfile.ZipFile(TESTFN2, "r", self.compression) as zipfp:
            ukijumuisha zipfp.open("strfile") as openobj:
    ...
    eleza test_read_return_size(self):
        ...
        kila test_size kwenye (1, 4095, 4096, 4097, 16384):
            ...
            ukijumuisha zipfile.ZipFile(io.BytesIO(), "w", self.compression) as zipf:
                zipf.writestr('foo', junk)
                ukijumuisha zipf.open('foo', 'r') as fp:
                    buf = fp.read(test_size)
                    self.assertEqual(len(buf), test_size)

    eleza test_truncated_zipfile(self):
        fp = io.BytesIO()
        ukijumuisha zipfile.ZipFile(fp, mode='w') as zipf:
            zipf.writestr('strfile', self.data, compress_type=self.compression)
            end_offset = fp.tell()
        zipfiledata = fp.getvalue()

        fp = io.BytesIO(zipfiledata)
        ukijumuisha zipfile.ZipFile(fp) as zipf:
            ukijumuisha zipf.open('strfile') as zipopen:
    ...
    eleza test_repr(self):
        fname = 'file.name'
        kila f kwenye get_files(self):
            ukijumuisha zipfile.ZipFile(f, 'w', self.compression) as zipfp:
            ...
            ukijumuisha zipfile.ZipFile(f, 'r') as zipfp:
                ...
                ukijumuisha zipfp.open(fname) as zipopen:
                    r = repr(zipopen)
    ...
    eleza test_per_file_compresslevel(self):
        ...
        compression levels."""
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", compresslevel=1) as zipfp:
    ...
    eleza test_writing_errors(self):
        ...
        wakati Kweli:
            testfile = BrokenFile()
            count = Tupu
            ukijumuisha zipfile.ZipFile(testfile, 'w', self.compression) as zipfp:
                ukijumuisha zipfp.open('file1', 'w') as f:
                    f.write(b'data1')
                count = 0
                jaribu:
                    ukijumuisha zipfp.open('file2', 'w') as f:
                        f.write(b'data2')
                except OSError:
                    stop += 1
                isipokua:
                ...
                mwishowe:
                    count = Tupu
            ukijumuisha zipfile.ZipFile(io.BytesIO(testfile.getvalue())) as zipfp:
...
kundi StoredTestsWithSourceFile(AbstractTestsWithSourceFile,
    ...
    eleza test_absolute_arcnames(self):
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", zipfile.ZIP_STORED) as zipfp:
            zipfp.write(TESTFN, "/absolute")

        ukijumuisha zipfile.ZipFile(TESTFN2, "r", zipfile.ZIP_STORED) as zipfp:
    ...
    eleza test_append_to_zip_file(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, "a", zipfile.ZIP_STORED) as zipfp:
            zipfp.writestr("strfile", self.data)
    ...
    eleza test_append_to_non_zip_file(self):
        ...
        ukijumuisha open(TESTFN2, 'wb') as f:
            f.write(data)
        ...
        ukijumuisha open(TESTFN2, 'rb') as f:
            ...
            zipfiledata = f.read()
        ukijumuisha io.BytesIO(zipfiledata) as bio, zipfile.ZipFile(bio) as zipfp:
            self.assertEqual(zipfp.namelist(), [TESTFN])
            self.assertEqual(zipfp.read(TESTFN), self.data)

    eleza test_read_concatenated_zip_file(self):
        ukijumuisha io.BytesIO() as bio:
            ukijumuisha zipfile.ZipFile(bio, 'w', zipfile.ZIP_STORED) as zipfp:
                zipfp.write(TESTFN, TESTFN)
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2) as zipfp:
            self.assertEqual(zipfp.namelist(), [TESTFN])
            self.assertEqual(zipfp.read(TESTFN), self.data)
    ...
    eleza test_append_to_concatenated_zip_file(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, 'a') as zipfp:
            self.assertEqual(zipfp.namelist(), [TESTFN])
            zipfp.writestr('strfile', self.data)
    ...
    eleza test_ignores_newline_at_end(self):
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", zipfile.ZIP_STORED) as zipfp:
            zipfp.write(TESTFN, TESTFN)
        ukijumuisha open(TESTFN2, 'a') as f:
            f.write("\r\n\00\00\00")
        ukijumuisha zipfile.ZipFile(TESTFN2, "r") as zipfp:
    ...
    eleza test_write_default_name(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, "w") as zipfp:
            zipfp.write(TESTFN)
    ...
    eleza test_write_to_readonly(self):
        ...
        raises a ValueError."""
        ukijumuisha zipfile.ZipFile(TESTFN2, mode="w") as zipfp:
            zipfp.writestr("somefile.txt", "bogus")

        ukijumuisha zipfile.ZipFile(TESTFN2, mode="r") as zipfp:
    ...
    eleza test_add_file_before_1980(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", strict_timestamps=Uongo) as zipfp:
            zipfp.write(TESTFN)
...
kundi AbstractTestZip64InSmallFiles:
    ...
    eleza zip_test(self, f, compression):
        # Create the ZIP archive
        ukijumuisha zipfile.ZipFile(f, "w", compression, allowZip64=Kweli) as zipfp:
            zipfp.write(TESTFN, "another.name")
            zipfp.write(TESTFN, TESTFN)
            zipfp.writestr("strfile", self.data)
...
kundi StoredTestZip64InSmallFiles(AbstractTestZip64InSmallFiles,
    ...
    eleza large_file_exception_test(self, f, compression):
        ukijumuisha zipfile.ZipFile(f, "w", compression, allowZip64=Uongo) as zipfp:
    ...
    eleza test_append(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", allowZip64=Kweli) as zipfp:
            zipfp.writestr("strfile", self.data)
        ukijumuisha zipfile.ZipFile(TESTFN2, "r", allowZip64=Kweli) as zipfp:
            zinfo = zipfp.getinfo("strfile")
            extra = zinfo.extra
        ukijumuisha zipfile.ZipFile(TESTFN2, "a", allowZip64=Kweli) as zipfp:
...
kundi AbstractWriterTests:
    ...
    eleza test_close_after_close(self):
        data = b'content'
        ukijumuisha zipfile.ZipFile(TESTFN2, "w", self.compression) as zipf:
            w = zipf.open('test', 'w')
            w.write(data)
            w.close()
...
kundi PyZipFileTests(unittest.TestCase):
    eleza assertCompiledIn(self, name, namelist):
        ikiwa name + 'o' sio kwenye namelist:
    ...
    eleza test_write_pyfile(self):
        ...
        ukijumuisha TemporaryFile() as t, zipfile.PyZipFile(t, "w") as zipfp:
            fn = __file__
    ...
    eleza test_write_filtered_python_package(self):
        ...
        ukijumuisha TemporaryFile() as t, zipfile.PyZipFile(t, "w") as zipfp:
            ...
            ukijumuisha captured_stdout() as reportSIO:
                zipfp.writepy(packagedir)
            ...
            reportStr = reportSIO.getvalue()
            self.assertKweli('SyntaxError' sio kwenye reportStr)

            # then check that the filter works on individual files
            eleza filter(path):
                rudisha sio os.path.basename(path).startswith("bad")
            ukijumuisha captured_stdout() as reportSIO, self.assertWarns(UserWarning):
    ...
    eleza test_write_with_optimization(self):
        ...
        ukijumuisha TemporaryFile() as t, \
    ...
    eleza test_write_python_directory(self):
        os.mkdir(TESTFN2)
        jaribu:
            ukijumuisha open(os.path.join(TESTFN2, "mod1.py"), "w") as fp:
                fp.write("andika(42)\n")

            ukijumuisha open(os.path.join(TESTFN2, "mod2.py"), "w") as fp:
                fp.write("andika(42 * 42)\n")

            ukijumuisha open(os.path.join(TESTFN2, "mod2.txt"), "w") as fp:
    ...
    eleza test_write_pyfile_bad_syntax(self):
        ...
        jaribu:
            ...
            ukijumuisha TemporaryFile() as t, zipfile.PyZipFile(t, "w") as zipfp:
                ...
                # as it will sio have compiled the python file, it will
...
kundi ExtractTests(unittest.TestCase):
        ...
        ukijumuisha temp_cwd():
            ...
            ukijumuisha zipfile.ZipFile(TESTFN2, "r") as zipfp:
                kila fpath, fdata kwenye SMALL_TEST_DATA:
                    ...
                    ukijumuisha open(writtenfile, "rb") as f:
    ...
    eleza test_extract_with_target(self):
        ukijumuisha temp_dir() as extdir:
        ...
        ukijumuisha temp_cwd():
            ...
            ukijumuisha zipfile.ZipFile(TESTFN2, "r") as zipfp:
                ...
                kila fpath, fdata kwenye SMALL_TEST_DATA:
                    ...
                    ukijumuisha open(outfile, "rb") as f:
                        self.assertEqual(fdata.encode(), f.read())
...
kundi OtherTests(unittest.TestCase):
    eleza test_open_via_zip_info(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, "r") as zipfp:
            ...
            kila info kwenye infos:
                ukijumuisha zipfp.open(info) as zipopen:
                    data += zipopen.read()
    ...
    eleza test_writestr_extended_local_header_issue1202(self):
        ukijumuisha zipfile.ZipFile(TESTFN2, 'w') as orig_zip:
    ...
    eleza test_close_on_exception(self):
        """Check that the zipfile ni closed ikiwa an exception ni raised kwenye the
        'with' block."""
        ukijumuisha zipfile.ZipFile(TESTFN2, "w") as zipfp:
            kila fpath, fdata kwenye SMALL_TEST_DATA:
                zipfp.writestr(fpath, fdata)
        ...
        jaribu:
            ukijumuisha zipfile.ZipFile(TESTFN2, "r") as zipfp2:
    ...
    eleza test_read_unicode_filenames(self):
        ...
        ukijumuisha zipfile.ZipFile(fname) as zipfp:
            kila name kwenye zipfp.namelist():
                zipfp.open(name).close()

    eleza test_write_unicode_filenames(self):
        ukijumuisha zipfile.ZipFile(TESTFN, "w") as zf:
        ...
        ukijumuisha zipfile.ZipFile(TESTFN, "r") as zf:
    ...
    eleza test_exclusive_create_zip_file(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, "x", zipfile.ZIP_STORED) as zipfp:
    ...
    eleza test_create_non_existent_file_for_append(self):
        ...
        jaribu:
            ukijumuisha zipfile.ZipFile(TESTFN, 'a') as zf:
    ...
    eleza test_close_erroneous_file(self):
        ...
        #
        ukijumuisha open(TESTFN, "w") as fp:
    ...
    eleza test_damaged_zipfile(self):
        ...
        #   a BadZipFile exception ni raised when we try to open it
    ...
    eleza test_is_zip_valid_file(self):
        ...
        # - passing a filename
        ukijumuisha zipfile.ZipFile(TESTFN, mode="w") as zipf:
            zipf.writestr("foo.txt", b"O, kila a Muse of Fire!")
    ...
    eleza test_closed_zip_raises_ValueError(self):
        ...
        data = io.BytesIO()
        ukijumuisha zipfile.ZipFile(data, mode="w") as zipf:
            zipf.writestr("foo.txt", "O, kila a Muse of Fire!")

        # This ni correct; calling .read on a closed ZipFile should raise
    ...
    eleza test_bad_open_mode(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN, mode="r") as zipf:
    ...
    eleza test_read0(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN, mode="w") as zipf:
            zipf.writestr("foo.txt", "O, kila a Muse of Fire!")
            # read the data to make sure the file ni there
            ukijumuisha zipf.open("foo.txt") as f:
                kila i kwenye range(FIXEDTEST_SIZE):
    ...
    eleza test_open_non_existent_item(self):
        ...
        exist kwenye the archive raises a RuntimeError."""
        ukijumuisha zipfile.ZipFile(TESTFN, mode="w") as zipf:
    ...
    eleza test_unsupported_compression(self):
        # data ni declared as shrunk, but actually deflated
        ...
        ukijumuisha zipfile.ZipFile(io.BytesIO(data), 'r') as zipf:
    ...
    eleza test_comments(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN, mode="r") as zipfr:
        ...
        ukijumuisha zipfile.ZipFile(TESTFN,mode="w") as zipf:
            ...
            zipf.writestr("foo.txt", "O, kila a Muse of Fire!")
        ukijumuisha zipfile.ZipFile(TESTFN,mode="a") as zipf:
    ...
    eleza test_unicode_comment(self):
        ukijumuisha zipfile.ZipFile(TESTFN, "w", zipfile.ZIP_STORED) as zipf:
            zipf.writestr("foo.txt", "O, kila a Muse of Fire!")
            ukijumuisha self.assertRaises(TypeError):
                zipf.comment = "this ni an error"

    eleza test_change_comment_in_empty_archive(self):
        ukijumuisha zipfile.ZipFile(TESTFN, "a", zipfile.ZIP_STORED) as zipf:
            self.assertUongo(zipf.filelist)
            zipf.comment = b"this ni a comment"
        ukijumuisha zipfile.ZipFile(TESTFN, "r") as zipf:
    ...
    eleza test_zipfile_with_short_extra_field(self):
        ...
        ukijumuisha zipfile.ZipFile(io.BytesIO(zipdata), 'r') as zipf:
            # testzip returns the name of the first corrupt file, ama Tupu
    ...
    eleza test_open_conflicting_handles(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, 'w', zipfile.ZIP_STORED) as zipf:
            ukijumuisha zipf.open('foo', mode='w') as w2:
                w2.write(msg1)
            ukijumuisha zipf.open('bar', mode='w') as w1:
                ukijumuisha self.assertRaises(ValueError):
                ...
                ukijumuisha self.assertRaises(ValueError):
                    zipf.close()
                w1.write(msg2)
            ukijumuisha zipf.open('baz', mode='w') as w2:
                w2.write(msg3)

        ukijumuisha zipfile.ZipFile(TESTFN2, 'r') as zipf:
    ...
    eleza test_seek_tell(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN, "w") as zipf:
            zipf.writestr("foo.txt", txt)
        ukijumuisha zipfile.ZipFile(TESTFN, "r") as zipf:
            ukijumuisha zipf.open("foo.txt", "r") as fp:
        ...
        ukijumuisha zipfile.ZipFile(data, mode="w") as zipf:
            zipf.writestr("foo.txt", txt)
        ukijumuisha zipfile.ZipFile(data, mode="r") as zipf:
            ukijumuisha zipf.open("foo.txt", "r") as fp:
                fp.seek(bloc, os.SEEK_SET)
                self.assertEqual(fp.tell(), bloc)
                fp.seek(-bloc, os.SEEK_CUR)
    ...
    eleza test_decompress_without_3rd_party_library(self):
        ...
        ukijumuisha zipfile.ZipFile(zip_file, 'w', compression=zipfile.ZIP_BZIP2) as zf:
            zf.writestr('a.txt', b'a')
        ukijumuisha mock.patch('zipfile.bz2', Tupu):
            ukijumuisha zipfile.ZipFile(zip_file) as zf:
...
kundi AbstractBadCrcTests:
    eleza test_testzip_with_bad_crc(self):
        ...
        ukijumuisha zipfile.ZipFile(io.BytesIO(zipdata), mode="r") as zipf:
            # testzip returns the name of the first corrupt file, ama Tupu
    ...
    eleza test_read_with_bad_crc(self):
        ...
        ukijumuisha zipfile.ZipFile(io.BytesIO(zipdata), mode="r") as zipf:
            ukijumuisha zipf.open('afile', 'r') as corrupt_file:
...
kundi DecryptionTests(unittest.TestCase):
    ...
    eleza setUp(self):
        ...
        ukijumuisha open(TESTFN2, "wb") as fp:
            fp.write(self.data2)
    ...
    eleza test_writestr(self):
        kila wrapper kwenye (lambda f: f), Tellable, Unseekable:
            ukijumuisha self.subTest(wrapper=wrapper):
                ...
                ukijumuisha zipfile.ZipFile(wrapper(bf), 'w', zipfile.ZIP_STORED) as zipfp:
                    zipfp.writestr('ones', b'111')
                    zipfp.writestr('twos', b'222')
                self.assertEqual(f.getvalue()[:5], b'abcPK')
                ukijumuisha zipfile.ZipFile(f, mode='r') as zipf:
                    ukijumuisha zipf.open('ones') as zopen:
                        self.assertEqual(zopen.read(), b'111')
                    ukijumuisha zipf.open('twos') as zopen:
        ...
        kila wrapper kwenye (lambda f: f), Tellable, Unseekable:
            ukijumuisha self.subTest(wrapper=wrapper):
                ...
                ukijumuisha zipfile.ZipFile(wrapper(bf), 'w', zipfile.ZIP_STORED) as zipfp:
                    ...
                    ukijumuisha open(TESTFN, 'wb') as f2:
                        f2.write(b'111')
    ...
    eleza test_open_write(self):
        kila wrapper kwenye (lambda f: f), Tellable, Unseekable:
            ukijumuisha self.subTest(wrapper=wrapper):
                ...
                f.write(b'abc')
                bf = io.BufferedWriter(f)
                ukijumuisha zipfile.ZipFile(wrapper(bf), 'w', zipfile.ZIP_STORED) as zipf:
                    ukijumuisha zipf.open('ones', 'w') as zopen:
                        zopen.write(b'111')
                    ukijumuisha zipf.open('twos', 'w') as zopen:
                        zopen.write(b'222')
                self.assertEqual(f.getvalue()[:5], b'abcPK')
                ukijumuisha zipfile.ZipFile(f) as zipf:
...
kundi TestsWithMultipleOpens(unittest.TestCase):
    ...
    eleza make_test_archive(self, f):
        # Create the ZIP archive
        ukijumuisha zipfile.ZipFile(f, "w", zipfile.ZIP_DEFLATED) as zipfp:
    ...
    eleza test_same_file(self):
        ...
        kila f kwenye get_files(self):
            ...
            ukijumuisha zipfile.ZipFile(f, mode="r") as zipf:
                ukijumuisha zipf.open('ones') as zopen1, zipf.open('ones') as zopen2:
    ...
    eleza test_different_file(self):
        ...
        kila f kwenye get_files(self):
            self.make_test_archive(f)
            ukijumuisha zipfile.ZipFile(f, mode="r") as zipf:
                ukijumuisha zipf.open('ones') as zopen1, zipf.open('twos') as zopen2:
                    data1 = zopen1.read(500)
                    data2 = zopen2.read(500)
                    data1 += zopen1.read()
                    data2 += zopen2.read()
    ...
    eleza test_interleaved(self):
        ...
        kila f kwenye get_files(self):
            self.make_test_archive(f)
            ukijumuisha zipfile.ZipFile(f, mode="r") as zipf:
                ukijumuisha zipf.open('ones') as zopen1:
                    data1 = zopen1.read(500)
                    ukijumuisha zipf.open('twos') as zopen2:
                        data2 = zopen2.read(500)
                        data1 += zopen1.read()
                        data2 += zopen2.read()
                self.assertEqual(data1, self.data1)
    ...
    eleza test_read_after_close(self):
        kila f kwenye get_files(self):
            ...
            ukijumuisha contextlib.ExitStack() as stack:
                ukijumuisha zipfile.ZipFile(f, 'r') as zipf:
    ...
    eleza test_read_after_write(self):
        kila f kwenye get_files(self):
            ukijumuisha zipfile.ZipFile(f, 'w', zipfile.ZIP_DEFLATED) as zipf:
    ...
    eleza test_many_opens(self):
        ...
        ukijumuisha zipfile.ZipFile(TESTFN2, mode="r") as zipf:
            kila x kwenye range(100):
                zipf.read('ones')
                ukijumuisha zipf.open('ones') as zopen1:
                    pass
        ukijumuisha open(os.devnull) as f:
            self.assertLess(f.fileno(), 100)

    eleza test_write_while_reading(self):
        ukijumuisha zipfile.ZipFile(TESTFN2, 'w', zipfile.ZIP_DEFLATED) as zipf:
            zipf.writestr('ones', self.data1)
        ukijumuisha zipfile.ZipFile(TESTFN2, 'a', zipfile.ZIP_DEFLATED) as zipf:
            ukijumuisha zipf.open('ones', 'r') as r1:
                data1 = r1.read(500)
                ukijumuisha zipf.open('twos', 'w') as w1:
                    w1.write(self.data2)
                data1 += r1.read()
        self.assertEqual(data1, self.data1)
        ukijumuisha zipfile.ZipFile(TESTFN2) as zipf:
...
kundi TestWithDirectory(unittest.TestCase):
    ...
    eleza test_extract_dir(self):
        ukijumuisha zipfile.ZipFile(findfile("zipdir.zip")) as zipf:
...
kundi CommandLineTest(unittest.TestCase):
    ...
    eleza test_list_command(self):
        zip_name = findfile('zipdir.zip')
        t = io.StringIO()
        ukijumuisha zipfile.ZipFile(zip_name, 'r') as tf:
            tf.printdir(t)
    ...
    eleza test_create_command(self):
        ...
        ukijumuisha open(os.path.join(TESTFNDIR, 'file.txt'), 'w') as f:
            f.write('test 2')
        ...
        kila opt kwenye '-c', '--create':
            jaribu:
                ...
                self.assertEqual(out, b'')
                ukijumuisha zipfile.ZipFile(TESTFN2) as zf:
    ...
    eleza test_extract_command(self):
        ...
        kila opt kwenye '-e', '--extract':
            ukijumuisha temp_dir() as extdir:
                ...
                self.assertEqual(out, b'')
                ukijumuisha zipfile.ZipFile(zip_name) as zf:
...
kundi TestExecutablePrependedZip(unittest.TestCase):
    ...
    eleza _test_zip_works(self, name):
        ...
        ukijumuisha zipfile.ZipFile(name) as zipfp:
...
kundi TestPath(unittest.TestCase):
    ...
    eleza zipfile_ondisk(self):
        ...
        kila alpharep kwenye self.zipfile_alpharep():
            ...
            ukijumuisha path.open("wb") as strm:
