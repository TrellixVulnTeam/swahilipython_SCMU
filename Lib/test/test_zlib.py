...
agiza random
agiza sys
kutoka test.support agiza bigmemtest, _1G, _4G
...
kundi ChecksumTestCase(unittest.TestCase):
    # checksum test cases
    ...
    eleza test_crc32_adler32_unsigned(self):
    ...
    eleza test_same_as_binascii_crc32(self):
        foo = b'abcdefghijklmnop'
        crc = 2486878355
...
kundi CompressTestCase(BaseCompressTestCase, unittest.TestCase):
    ...
    eleza test_big_compress_buffer(self, size):
        compress = lambda s: zlib.compress(s, 1)
        self.check_big_compress_buffer(size, compress)

    @bigmemtest(size=_1G + 1024 * 1024, memuse=2)
    eleza test_big_decompress_buffer(self, size):
        self.check_big_decompress_buffer(size, zlib.decompress)

    @bigmemtest(size=_4G, memuse=1)
    eleza test_large_bufsize(self, size):
    ...
    eleza test_custom_bufsize(self):
        data = HAMLET_SCENE * 10
        compressed = zlib.compress(data, 1)
...
kundi CompressObjectTestCase(BaseCompressTestCase, unittest.TestCase):
    ...
    eleza test_compressoptions(self):
    ...
    eleza test_maxlen_custom(self):
        data = HAMLET_SCENE * 10
        compressed = zlib.compress(data, 1)
        dco = zlib.decompressobj()
        self.assertEqual(dco.decompress(compressed, CustomInt()), data[:100])

    eleza test_clear_unconsumed_tail(self):
    ...
    # issue27164
    eleza test_decompress_raw_with_dictionary(self):
    ...
    @bigmemtest(size=_4G, memuse=1)
    eleza test_flush_large_length(self, size):
    ...
    eleza test_flush_custom_length(self):
        input = HAMLET_SCENE * 10
        data = zlib.compress(input, 1)
        dco = zlib.decompressobj()
        dco.decompress(data, 1)
    ...
    eleza test_decompresscopy(self):
    ...
    eleza test_large_unused_data(self, size):
