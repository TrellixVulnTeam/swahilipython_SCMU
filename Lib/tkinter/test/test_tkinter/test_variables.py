agiza unittest
agiza gc
kutoka tkinter agiza (Variable, StringVar, IntVar, DoubleVar, BooleanVar, Tcl,
                     TclError)
...
kundi TestVariable(TestBase):

    eleza info_exists(self, *args):
        rudisha self.root.getboolean(self.root.call("info", "exists", *args))
    ...
    eleza test_name_and_value(self):
        v = Variable(self.root, "sample string", "varname")
        self.assertEqual("sample string", v.get())
        self.assertEqual("varname", str(v))

    eleza test___del__(self):
        self.assertUongo(self.info_exists("varname"))
        v = Variable(self.root, "sample string", "varname")
        self.assertKweli(self.info_exists("varname"))
        toa v
        self.assertUongo(self.info_exists("varname"))

    eleza test_dont_unset_not_existing(self):
        self.assertUongo(self.info_exists("varname"))
    ...
    eleza test___eq__(self):
        # values doesn't matter, only kundi na name are checked
    ...
    eleza test_initialize(self):
        v = Var(self.root)
        self.assertUongo(v.side_effect)
        v.set("value")
        self.assertKweli(v.side_effect)

    eleza test_trace_old(self):
        # Old interface
        v = Variable(self.root)
        vname = str(v)
        trace = []
        eleza read_tracer(*args):
            trace.append(('read',) + args)
        eleza write_tracer(*args):
...
kundi TestIntVar(TestBase):
    ...
    eleza test_invalid_value(self):
        v = IntVar(self.root, name="name")
        self.root.globalsetvar("name", "value")
        ukijumuisha self.assertRaises((ValueError, TclError)):
            v.get()


kundi TestDoubleVar(TestBase):
...
kundi TestBooleanVar(TestBase):

    eleza test_default(self):
        v = BooleanVar(self.root)
        self.assertIs(v.get(), Uongo)

    eleza test_get(self):
        v = BooleanVar(self.root, Kweli, "name")
        self.assertIs(v.get(), Kweli)
        self.root.globalsetvar("name", "0")
        self.assertIs(v.get(), Uongo)
        self.root.globalsetvar("name", 42 ikiwa self.root.wantobjects() isipokua 1)
        self.assertIs(v.get(), Kweli)
        self.root.globalsetvar("name", 0)
        self.assertIs(v.get(), Uongo)
    ...
    eleza test_set(self):
        true = 1 ikiwa self.root.wantobjects() isipokua "1"
        false = 0 ikiwa self.root.wantobjects() isipokua "0"
    ...
    eleza test_invalid_value_domain(self):
        false = 0 ikiwa self.root.wantobjects() isipokua "0"
        v = BooleanVar(self.root, name="name")
        ukijumuisha self.assertRaises(TclError):
            v.set("value")
