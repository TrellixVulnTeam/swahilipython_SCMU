...
eleza _tp_cache(func):
    ...
    eleza inner(*args, **kwds):
        ...
        tatizo TypeError:
            pita  # All real errors (sio unhashable args) are raised below.
...
eleza get_type_hints(obj, globalns=Tupu, localns=Tupu):
    ...
    are returned kama a dictionary. For classes, annotations include also
    inherited members.
...
kundi NamedTuple(metaclass=NamedTupleMeta):
    ...
    dict that maps field names to types.  (The field names are also kwenye
