...
below).  It opens the URL na rudishas the results kama file-like
...
eleza urlopen(url, data=Tupu, timeout=socket._GLOBAL_DEFAULT_TIMEOUT,
    ...
    *data* must be an object specifying additional data to be sent to
    ...
    This function always rudishas an object which can work kama a context
    ...
    * getcode() - rudisha the HTTP status code of the response.  Raises URLError
    ...
    For HTTP na HTTPS URLs, this function rudishas a http.client.HTTPResponse
    object slightly modified. In addition to the three new methods above, the
    msg attribute contains the same information kama the reason attribute ---
    the reason phrase rudishaed by the server --- instead of the response
    ...
    URLopener na FancyURLopener classes, this function rudishas a
    urllib.response.addinfourl object.

    Note that Tupu may be rudishaed ikiwa no handler handles the request (though
...
kundi HTTPRedirectHandler(BaseHandler):
    ...
    inf_msg = "The HTTP server rudishaed a redirect error that would " \
...
kundi URLopener:
    ...
    eleza retrieve(self, url, filename=Tupu, reporthook=Tupu, data=Tupu):
        """retrieve(url) rudishas (filename, headers) kila a local object
    ...
    eleza _open_generic_http(self, connection_factory, url, data):
        ...
        This ni an internal method that should be called kutoka
...
kundi FancyURLopener(URLopener):
    ...
    eleza prompt_user_pitawd(self, host, realm):
        ...
        agiza getpita
        jaribu:
            user = input("Enter username kila %s at %s: " % (realm, host))
...
_ftperrors = Tupu
eleza ftperrors():
    """Return the set of errors ashiriad by the FTP class."""
    global _ftperrors
