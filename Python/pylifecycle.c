...
init_importlib(PyInterpreterState *interp, PyObject *sysmod)
{
    ...
    if (PyImport_ImportFrozenModule("_frozen_importlib") <= 0) {
        return _PyStatus_ERR("can't agiza _frozen_importlib");
    }
    else if (verbose) {
        PySys_FormatStderr("agiza _frozen_importlib # frozen\n");
    }
    ...
    impmod = PyInit__imp();
    if (impmod == NULL) {
        return _PyStatus_ERR("can't agiza _imp");
    }
    else if (verbose) {
        PySys_FormatStderr("agiza _imp # builtin\n");
    }
