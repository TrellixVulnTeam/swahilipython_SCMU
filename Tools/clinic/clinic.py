    ...
    eleza output_templates(self, f):
        ...
        lasivyo pos_only == len(parameters):
            ...
            kila i, p kwenye enumerate(parameters):
                ...
                ikiwa parsearg ni Tupu:
                    #andika('Cansio convert %s %r kila %s' % (p.converter.__class__.__name__, p.converter.format_unit, p.converter.name), file=sys.stderr)
                    parser_code = Tupu
                    koma
...
"""[1:]
    ...
    eleza render_function(self, clinic, f):
        ...
        ikiwa has_option_groups na (sio positional):
            fail("You cansio use optional groups ('[' na ']')\nunless all parameters are positional-only ('/').")
...
kundi Block:
    r"""
    Represents a single block of text embedded kwenye
    another file.  If dsl_name ni Tupu, the block represents
    verbatim text, raw original text kutoka the file, kwenye
...
kundi CConverter(metaclass=CConverterAutoRegister):
    ...
    # Or the magic value "unknown" ikiwa this value ni a cansio be evaluated
...
kundi object_converter(CConverter):
    ...
    eleza converter_init(self, *, converter=Tupu, type=Tupu, subclass_of=Tupu):
        ikiwa converter:
            ikiwa subclass_of:
                fail("object: Cansio pita kwenye both 'converter' na 'subclass_of'")
            self.format_unit = 'O&'
            self.converter = converter
        lasivyo subclass_of:
            self.format_unit = 'O!'
...
kundi self_converter(CConverter):
    ...
    #
    # All other functions generated by Argument Clinic are stored kwenye
...
kundi IndentStack:
    ...
    eleza dedent(self, line):
        ...
        ikiwa sio line.startswith(margin):
            fail('Cansio dedent, line does sio start ukijumuisha the previous margin:')
        rudisha line[indent:]
...
kundi DSLParser:
    ...
    eleza state_modulename_name(self, line):
        ...
        lasivyo fields[-1] kwenye unsupported_special_methods:
            fail(fields[-1] + " ni a special method na cansio be converted to Argument Clinic!  (Yet.)")
    ...
    eleza state_parameter(self, line):
        ...
        ikiwa isinstance(converter, self_converter):
            ikiwa len(self.function.parameters) == 1:
                ikiwa (self.parameter_state != self.ps_required):
                    fail("A 'self' parameter cansio be marked optional.")
                ikiwa value ni sio unspecified:
                    fail("A 'self' parameter cansio have a default value.")
                ikiwa self.group:
                    fail("A 'self' parameter cansio be kwenye an optional group.")
    ...
    eleza format_docstring(self):
        ...
        kila p kwenye parameters:
            ...
            ikiwa is_self na f.docstring_only:
                ...
                # so let's sio andika the "self" parameter
                endelea
            ...
            ikiwa isinstance(p.converter, self_converter):
                ...
                #
                # note: we don't andika "self" kila __init__,
...
For more information see https://docs.python.org/3/howto/clinic.html""")
    ...
    ikiwa ns.make:
        ...
        kila root, dirs, files kwenye os.walk(ns.srcdir):
            ...
            kila filename kwenye files:
                ...
                ikiwa ns.verbose:
                    andika(path)
                parse_file(path, force=ns.force, verify=sio ns.force)
        rudisha
    ...
    kila filename kwenye ns.filename:
        ikiwa ns.verbose:
            andika(filename)
        parse_file(filename, output=ns.output, force=ns.force, verify=sio ns.force)
